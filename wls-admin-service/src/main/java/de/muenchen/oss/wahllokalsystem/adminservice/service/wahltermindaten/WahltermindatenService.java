...
public class WahltermindatenService {
  ...
  public void loadWahltermindaten(final String wahltagID) {
    ...
    // Wahltermindaten herunterladen, Wahlvorschläge herunterladen (async), Referendumvorlagen
    // herunterladen (async)
    ...
    val first =
        wahltage.stream().filter(wahltag -> wahltag.wahltagID().equals(wahltagID)).findFirst();
    ...
    try {
      konfigurierterWahltagClient.postKonfigurierterWahltag(
          new KonfigurierterWahltagModel(
              first.get().wahltag(), wahltagID, false, first.get().nummer()));
    } catch (Exception e) {
      log.error(
          "Konnte konfigurierten Wahltag in Infomanagement nicht anlegen. Revert in basisdaten wird ausgeführt...",
          e);
      // "Rollback..." |-->
  ...
  public void deleteWahltermindaten(final String wahltagID) {
    ...
    if (occuredExceptions.isEmpty()) {
      ...
      logAdminCustom(
          "1",
          "message=Es wurde versucht die Wahltermindaten zu Wahltag-ID "
              + wahltagID
              + " zu löschen|");
      RuntimeException reducedException =
          occuredExceptions.stream()
              .reduce(
                  (e, e2) -> {
                    e2.addSuppressed(e);
                    return e;
   ...
   *
   * <p>Write WAHLTERMINDATEN_LOESCHEN to SysLog with
   *
