...
class AWerteClientImplTest {
  ...
  @Mock AWerteControllerApi awerteControllerApi;

  @Mock ExceptionFactory exceptionFactory;

  @InjectMocks AWerteClientImpl unitUnderTest;
    ...
    void should_rethrowWlsException_when_wlsExceptionIsThrownFromAWerteApi() {
      ...
      val mockedWlsException =
          TechnischeWlsException.withCode("000")
              .buildWithMessage("communication with AWerte api failed");

      Mockito.doThrow(mockedWlsException)
          .when(awerteControllerApi)
          .initialiseAWerte(wahlbezirkIDList);

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.initialiseAWerte(wahlbezirkIDList))
