...
class WahllokalBenutzerServiceTest {
  ...
  @Mock WahllokalBenutzerValidator wahllokalBenutzerValidator;

  @Mock WahllokalBenutzerClient wahllokalBenutzerClient;

  @Mock WahlbezirkeClient wahlbezirkeClient;

  @InjectMocks WahllokalBenutzerService unitUnderTest;

  @Captor ArgumentCaptor<List<WahllokalBenutzerModel>> userModelsCaptor;
  ...
  class GenerateWahllokalbenutzer {
    ...
    void should_throwException_when_wahltagIDIsInvalid() {
      ...
      Mockito.doThrow(mockedValidationException)
          .when(wahllokalBenutzerValidator)
          .validWahltagIDParamOrThrow(wahltagID);

      Assertions.assertThatThrownBy(() -> unitUnderTest.generateWahllokalbenutzer(wahltagID))
    ...
    void should_generateWahllokalBenutzerModels_when_givenWahltagIDAndWahlbezirke() {
      ...
      Mockito.verify(wahllokalBenutzerClient)
          .generateAndExportWahllokalBenutzer(eq(wahltagID), userModelsCaptor.capture());

      val expectedUserModels = getListOfWahlLokalBenutzerModels();
      Assertions.assertThat(userModelsCaptor.getValue())
    ...
    @Test
    void
        should_setTheUserWahlbezirkIDOfTheWahlWithLowestWahlnummer_when_givenWahltagIDAndWahlbezirke() {
      val wahltagID = "wahltagID";
      val mockedWahlbezirke = getMockWahlbezirke();
      ...
      for (WahllokalBenutzerModel benutzer : generatedUsers) {
        String minWahlbezirkID =
            benutzer.wbid_wahlnummer().stream()
    ...
    void should_exportCsvFileModelWithWahllokalbenutzerNames_when_givenWahltagIDAndWahlbezirke() {
      ...
      Mockito.when(
          ...
          .thenReturn(mockedWahllokalBenutzer);
  ...
  class ExportWahllokalBenutzer {
    ...
    void should_throwException_when_wahltagIDIsInvalid() {
      ...
      Assertions.assertThatThrownBy(() -> unitUnderTest.exportWahllokalBenutzer(wahltagID))
          .isSameAs(mockedValidationException);
    }
    ...
    void should_exportWahllokalBenutzer_when_givenWahltagID() {
      ...
      Mockito.doNothing().when(wahllokalBenutzerValidator).validWahltagIDParamOrThrow(wahltagID);
      Mockito.when(wahllokalBenutzerClient.exportWahllokalBenutzer(wahltagID))
          .thenReturn(mockedWahllokalBenutzer);
  ...
  class DeleteWahllokalBenutzer {
    ...
    void should_throwException_when_wahltagIDIsInvalid() {
      ...
      Assertions.assertThatThrownBy(() -> unitUnderTest.deleteWahllokalBenutzer(wahltagID))
          .isSameAs(mockedValidationException);
    }
  ...
  private final String mockedWahllokalBenutzer =
      "ftpprs-1503\r\n" + "c94m3c-0365\r\n" + "v7jnkr-2161\r\n";

  private List<WahlbezirkModel> getMockWahlbezirke() {
    return List.of(
        new WahlbezirkModel(
            "wahlbezirkID2_0", WahlbezirkArtModel.BWB, "0365", LocalDate.now(), "0", "wahlID0"),
        new WahlbezirkModel(
            "wahlbezirkID3_0", WahlbezirkArtModel.UWB, "2161", LocalDate.now(), "0", "wahlID0"),
        new WahlbezirkModel(
            "wahlbezirkID1_1", WahlbezirkArtModel.UWB, "1503", LocalDate.now(), "1", "wahlID1"),
        new WahlbezirkModel(
            "wahlbezirkID1_2", WahlbezirkArtModel.UWB, "1503", LocalDate.now(), "2", "wahlID2"),
        new WahlbezirkModel(
            "wahlbezirkID2_1", WahlbezirkArtModel.BWB, "0365", LocalDate.now(), "1", "wahlID1"),
        new WahlbezirkModel(
            "wahlbezirkID2_2", WahlbezirkArtModel.BWB, "0365", LocalDate.now(), "2", "wahlID2"),
        new WahlbezirkModel(
            "wahlbezirkID3_1", WahlbezirkArtModel.UWB, "2161", LocalDate.now(), "1", "wahlID1"),
        new WahlbezirkModel(
            "wahlbezirkID3_2", WahlbezirkArtModel.UWB, "2161", LocalDate.now(), "2", "wahlID2"),
        new WahlbezirkModel(
            "wahlbezirkID1_0", WahlbezirkArtModel.UWB, "1503", LocalDate.now(), "0", "wahlID0"));
  }

  private List<WahllokalBenutzerModel> getListOfWahlLokalBenutzerModels() {
    val wahltag = LocalDate.now();
    val wahllokalBenutzerModel_1 =
