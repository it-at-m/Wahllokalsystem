...
class WahllokalBenutzerValidatorTest {
  ...
  @InjectMocks private WahllokalBenutzerValidator unitUnderTest;
  ...
  class WahlbezirkeExistOrThrow {
    ...
    void should_throwException_when_wahlbezirkeNull(
        final List<WahlbezirkModel> wahlbezirkModels, final String description) {
      val mockedException =
          TechnischeWlsException.withCode("165")
              .buildWithMessage("Kein Wahltag vorhanden für die angegebene Wahltag-ID");
      Mockito.when(
              exceptionFactory.createTechnischeWlsException(ExceptionConstants.INVALID_ARGUMENT))
          .thenReturn(mockedException);

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.wahlbezirkeExistOrThrow(wahlbezirkModels))
          .isSameAs(mockedException);
    }

    @Test
    void should_notThrowException_when_wahlbezirkeExistOrThrow() {
      WahlbezirkModel wahlbezirkModel =
          new WahlbezirkModel("id", WahlbezirkArtModel.BWB, "3", LocalDate.now(), "3", "4");
      val listOfWahlbezirkModels = List.of(wahlbezirkModel);

      Assertions.assertThatNoException()
          .isThrownBy(() -> unitUnderTest.wahlbezirkeExistOrThrow(listOfWahlbezirkModels));
    }
