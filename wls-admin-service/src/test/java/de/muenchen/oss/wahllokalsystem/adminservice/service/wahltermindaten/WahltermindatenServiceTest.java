...
class WahltermindatenServiceTest {
  ...
  @Mock WahltermindatenClient wahltermindatenClient;
  ...
  class LoadWahltermindaten {
    ...
    void should_throwTechnischeWlsException_when_wahltagIsNotFoundForWahltagID() {
      ...
      val mockedTechnischeWlsException =
          TechnischeWlsException.withCode("000")
              .buildWithMessage("Wahltag not found for wahltagID");

      Mockito.when(
              exceptionFactory.createTechnischeWlsException(ExceptionConstants.INVALID_ARGUMENT))
    ...
    void should_postKonfigurierterWahltag_when_aWerteAreInitialised() {
      val wahltagID = "wahltagID";
      val mockedWahltagModel =
          new WahltagModel(wahltagID, LocalDate.now(), "beschreibung", "nummer");
      val mockedWahltagList = List.of(mockedWahltagModel);
      val mockedWahlbezirkModel =
          new WahlbezirkModel(
              "wahlbezirkID",
      ...
      Mockito.verify(konfigurierterWahltagClient)
          .postKonfigurierterWahltag(
              new KonfigurierterWahltagModel(
                  mockedWahltagModel.wahltag(),
                  mockedWahltagModel.wahltagID(),
                  false,
                  mockedWahltagModel.nummer()));
    }
    ...
    void should_doRollbackAndDeleteWahltermindaten_when_postKonfigurierterWahltagFails() {
      ...
      Mockito.doThrow(mockedException)
          ...
          .postKonfigurierterWahltag(
              new KonfigurierterWahltagModel(
                  mockedWahltag.wahltag(),
                  mockedWahltag.wahltagID(),
                  false,
                  mockedWahltag.nummer()));
