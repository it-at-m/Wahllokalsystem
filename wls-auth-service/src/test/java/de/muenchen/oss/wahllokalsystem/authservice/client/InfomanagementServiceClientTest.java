...
class InfomanagementServiceClientTest {
  ...
  @Mock KonfigurationControllerApi konfigurationControllerApi;

  @Mock KonfigurierterWahltagControllerApi konfigurierterWahltagControllerApi;

  @InjectMocks InfomanagementServiceClient unitUnderTest;
  ...
  class GetWelcomeMessage {
    ...
    void should_returnMessageFromAPI_when_receivingConfigWithValue() {
      ...
      Mockito.when(konfigurationControllerApi.getKonfigurationUnauthorized(konfigKeyWelcomeMessage))
          .thenReturn(mockedKonfigurationDTO);
    ...
    void should_returnDefaultMessage_when_theApiThrowsAnException() {
      Mockito.doThrow(new RuntimeException("sth failed"))
          .when(konfigurationControllerApi)
          .getKonfigurationUnauthorized(konfigKeyWelcomeMessage);

      val result = unitUnderTest.getWelcomeMessage();

      Assertions.assertThat(result).isEqualTo(defaultWelcomeMessage);

      Mockito.verify(exceptionFactory)
          .createTechnischeWlsException(ExceptionConstants.KOMMUNIKATIONSFEHLER_MIT_KONFIGSERVICE);
    }
  }
  ...
  class GetLegalLoginInterval {
    ...
    void should_returnInterval_when_apiReturnValues() {
      ...
      Mockito.when(
              konfigurationControllerApi.getKonfigurationUnauthorized(KONFIG_KEY_EARLIEST_LOGIN))
      ...
      val expectedResult =
          new LegalLoginIntervalModel(
              LocalDateTime.parse(
                  earliestLoginDatetime, DateTimeFormatter.ofPattern(KONFIG_DATETIME_FORMAT)),
              LocalDateTime.parse(
                  latestLoginDatetime, DateTimeFormatter.ofPattern(KONFIG_DATETIME_FORMAT)));
    ...
    void should_mapToTechnischeWlsException_when_nonWlsExceptionIsThrownFromAPI() {
      val mockedTechnischeWlsException =
          TechnischeWlsException.withCode("")
              .buildWithMessage("konfiguration controller api call failed");

      Mockito.doThrow(new RuntimeException("konfiguration controller api call failed"))
          .when(konfigurationControllerApi)
  ...
  class IsWahltagActive {
    ...
    void should_returnTrue_when_apiReturnsTrue() {
      ...
      Mockito.when(konfigurierterWahltagControllerApi.isWahltagActive(wahltagID))
    ...
    void should_rethrowException_when_apiThrowsWlsException() {
      ...
      val mockedApiWlsException =
          TechnischeWlsException.withCode("").buildWithMessage("is wahltagActive call failed");
      Mockito.doThrow(mockedApiWlsException)
          .when(konfigurierterWahltagControllerApi)
          .isWahltagActive(wahltagID);

      Assertions.assertThatThrownBy(() -> unitUnderTest.isWahltagActive(wahltagID))
    ...
    void should_throwWlsException_when_apiThrowsNonWlsException() {
      ...
      val mockedWrappedWlsException =
          TechnischeWlsException.withCode("").buildWithMessage("is wahltagActive call failed");
      Mockito.doThrow(mockedApiNonWlsException)
          .when(konfigurierterWahltagControllerApi)
          .isWahltagActive(wahltagID);
      Mockito.when(
              exceptionFactory.createTechnischeWlsException(
      ...
      Assertions.assertThatThrownBy(() -> unitUnderTest.isWahltagActive(wahltagID))
          .isSameAs(mockedWrappedWlsException);
    }
  }
}
