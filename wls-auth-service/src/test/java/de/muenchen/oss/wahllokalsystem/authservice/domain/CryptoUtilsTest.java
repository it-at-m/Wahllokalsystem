...
class CryptoUtilsTest {
  ...
  @Mock ServiceIDFormatter idFormatter;

  @Mock Cipher cipher;

  @InjectMocks CryptoUtils unitUnderTest;
  ...
  class IsEncrypted {
    ...
    void should_returnTrue_when_valueStartsWithPrefix() {
      Assertions.assertThat(unitUnderTest.isEncrypted(ENCRYPTION_PREFIX + "the encrypted value"))
          .isTrue();
    }
    ...
    void should_returnFalse_when_valueDoesNotStartWithPrefix() {
      ...
      Assertions.assertThat(unitUnderTest.isEncrypted("values without encryption prefix"))
          .isFalse();
    }
  }
  ...
  class Encrypt {
    ...
    void should_returnEncryptedValueWithPrefix_when_valueIsGiven() throws Exception {
      ...
      val expectedResult =
          ENCRYPTION_PREFIX + Base64.getEncoder().encodeToString(mockedEncryptedValue);
    ...
    @ParameterizedTest
    @MethodSource(
        "de.muenchen.oss.wahllokalsystem.authservice.domain.CryptoUtilsTest#exceptionsMappedToWlsException")
    void should_throwTechnischeWlsException_when_cipherThrowsException(
        final Exception exceptionThrownByCipher) throws Exception {
      val valueToEncrypt = "hello world";
      ...
      val expectedException =
          TechnischeWlsException.withCode(ExceptionConstants.CRYPTO_EXCEPTION_CODE)
              .inService(mockedServiceID)
              .buildWithMessage("");
  ...
  class Decrypt {
    ...
    void should_returnDecryptedValue_when_valueIsGiven() throws Exception {
      ...
      val encryptedValueAsBase64WithPrefix =
          ENCRYPTION_PREFIX + Base64.getEncoder().encodeToString(encryptedValue.getBytes());
  ...
  public static Stream<Arguments> exceptionsMappedToWlsException() {
    ...
    return exceptions.stream()
        .map(exception -> Arguments.of(exception, exception.getClass().getName()));
  }
}
