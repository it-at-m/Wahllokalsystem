...
@SpringBootTest(
    ...
    properties = {"service.config.crypto.key=veryLongAndVerySaveKeyIHopeXXXabc123!!"})
...
class UserRepositoryImplIntegrationTest {
  ...
  class FindByWahltagID {
    ...
    void should_returnDecryptedUsername_when_gettingUserFromRepo() {
      ...
      Assertions.assertThat(result)
          .allSatisfy(
              user -> Assertions.assertThat(user.getUsername()).isEqualTo(USERNAME_UNENCRYPTED));
    }
    ...
    void should_keepTheUsernameEncrypted_when_gettingUserFromRepo() {
      ...
      Assertions.assertThat(userInRepoAfterFindBy.get().getUsername())
          .isEqualTo(USERNAME_ENCRYPTED);
    }
  }
          ...
          .isThrownBy(
              () ->
                  transactionTemplate.execute(
                      status -> userRepository.save(userToSaveWithSameUsername)))
    ...
    void should_throwException_when_multipleUsersHaveSameName() {
      ...
      Assertions.assertThatException()
          .isThrownBy(
              () ->
                  transactionTemplate.execute(
                      status -> userRepository.saveAll(List.of(user1, user2))))
          .isInstanceOf(DataIntegrityViolationException.class);
    }
  }
  ...
  class DeleteUsersByWahltagID {
    ...
    void should_deleteUsersWithWahltagIdButNotAuthoritiesOrPermissions_when_givenWahltagId() {
              ...
              status -> {
                ...
                val authority =
                    authorityRepository.save(
                        new Authority(
                            "authority1",
                            Set.of(permission1, permission2, permission3),
      ...
      transactionTemplate.executeWithoutResult(
          status -> userRepository.deleteUsersByWahltagID(wahltagID));

      Assertions.assertThat(savedUsers)
          .allSatisfy(
              user -> Assertions.assertThat(crudUserRepository.existsById(user.getId())).isFalse());
  ...
  class OnSchedule {
    ...
    void should_encryptExistingUsers_when_usersExists() {
      ...
      val savedSavedUnencryptedUser =
          transactionTemplate.execute(status -> crudUserRepository.save(userToEncrypt));

      transactionTemplate.executeWithoutResult(status -> userRepository.onSchedule());

      val userAfterEncryption =
          crudUserRepository.findById(savedSavedUnencryptedUser.getId()).get();
  ...
  private User createUser(
      final String username, final String wahltagID, final Authority authority) {
    val user = new User();
