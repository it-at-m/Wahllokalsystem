...
class UserRepositoryImplTest {
  ...
  @Mock CryptoUtils cryptoUtils;

  @Mock CrudUserRepository userRepository;

  @InjectMocks UserRepositoryImpl unitUnderTest;

  @Captor ArgumentCaptor<List<User>> usersCaptor;
  ...
  class OnInit {
    ...
    void should_updateWithNewUser_when_gotUsersFromRepository() {
      ...
      Assertions.assertThat(usersCaptor.getValue())
          .allSatisfy(
              user -> Assertions.assertThat(user.getUsername()).isEqualTo(mockedEncryptedUsername));
    }
  ...
  class FindByWahltagID {
    ...
    void should_decryptUsernames_when_foundUsers() {
      ...
      Assertions.assertThat(result)
          .allSatisfy(
              user -> Assertions.assertThat(user.getUsername()).isEqualTo(decryptedUsername));
    }
  ...
  class SaveAll {
    ...
    void should_callEncryptOnAllUser_when_savingMultipleUsers() {
      ...
      val username2 = "user2";
      val usersToSave =
          List.of(createUserWithUsername(username1), createUserWithUsername(username2));
      ...
      Assertions.assertThat(savedUsernames)
          .containsExactlyInAnyOrder(mockedEncryptedUsername1, mockedEncryptedUsername2);
    }
    ...
    void should_returnUnEncryptedUsername_when_gettingUsers() {
      ...
      Mockito.when(userRepository.saveAll(usersToSave))
          .then(invocationOnMock -> invocationOnMock.getArgument(0, List.class));

      val result = unitUnderTest.saveAll(usersToSave);
      val usernames =
          StreamSupport.stream(result.spliterator(), false).map(User::getUsername).toList();
  ...
  class FindByUsername {
    ...
    void should_returnUserWithDecryptedUsername_when_foundUser() {
      ...
      Mockito.when(userRepository.findByUsername(encryptedUsername))
  ...
  class IsLocked {
    ...
    void should_returnFalse_when_countResultIsZero() {
      ...
      val mockedEncryptedUsername = "encryptedUsername";
      Mockito.when(cryptoUtils.encrypt(username)).thenReturn(mockedEncryptedUsername);
      Mockito.when(userRepository.countUsersLockedByUsername(mockedEncryptedUsername))
          .thenReturn(0L);
