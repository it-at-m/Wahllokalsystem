...
import de.muenchen.oss.wahllokalsystem.authservice.service.OAuthServerSessions;
...
@SpringBootTest(
    ...
    properties = {"refarch.gracefulshutdown.pre-wait-seconds=0"})
...
class SessionControllerIntegrationTest {
    ...
    void should_findSessions_when_sessionsCreated() throws Exception {
      ...
      val responseBody =
          objectMapper.readValue(
              response.getResponse().getContentAsString(), OAuthServerSessions.class);
      val foundSessions = responseBody.getSessions();

      Assertions.assertThat(foundSessions)
          .filteredOn(
              session ->
                  session.getUsername().equals(session1.getAttribute("PRINCIPAL_NAME"))
                      || session.getUsername().equals(session2.getAttribute("PRINCIPAL_NAME"))
                      || session.getUsername().equals(session3.getAttribute("PRINCIPAL_NAME")))
          .hasSize(3);
    }
  }
  ...
  class KillSession {
    ...
    void should_killSession_when_killingSessionCalled() throws Exception {
      ...
      val responseReadSessions =
          api.perform(get("/oauthsessions/")).andExpect(status().isOk()).andReturn();
      val responseBody =
          objectMapper.readValue(
              responseReadSessions.getResponse().getContentAsString(), OAuthServerSessions.class);
      val foundSessions = responseBody.getSessions();
      // killing session 2
      val sessionIDToKill =
          foundSessions.stream()
              .filter(
                  session -> session.getUsername().equals(session2.getAttribute("PRINCIPAL_NAME")))
              .findFirst()
              .get()
              .getSessionId();
      val requestKillSession =
          post("/oauthsessions/" + sessionIDToKill + "/invalidate").with(csrf());
      api.perform(requestKillSession).andExpect(status().isOk()).andReturn();

      val responseReadSessionsAfterKill =
          api.perform(get("/oauthsessions/")).andExpect(status().isOk()).andReturn();
      val responseBodyAfterKill =
          objectMapper.readValue(
              responseReadSessionsAfterKill.getResponse().getContentAsString(),
              OAuthServerSessions.class);
      val foundSessionsAfterKill = responseBodyAfterKill.getSessions();
      // session killed in sessionRegistry
      Assertions.assertThat(foundSessionsAfterKill)
          .filteredOn(
              session ->
                  session.getUsername().equals(session1.getAttribute("PRINCIPAL_NAME"))
                      || session.getUsername().equals(session2.getAttribute("PRINCIPAL_NAME"))
  ...
  private Session createSpringSessionForUser(final String username) {
    ...
    if (sessionToSave.getAttribute("PRINCIPAL_NAME") != null && !sessionToSave.getId().isEmpty()) {
      sessionRegistry.registerNewSession(
          sessionToSave.getId(), sessionToSave.getAttribute("PRINCIPAL_NAME"));
