...
class ErrorMessageServiceTest {
    ...
    class getErrorMessage {
        ...
        @Test
        void should_returnTheMessageOfTheExceptionAfterFixedErrorMessage_when_exceptionIsInstanceOfSessionAuthenticationException() {
            val errorMessage = "bad credentials";
            val result = unitUnderTest.getErrorMessage(new SessionAuthenticationException(errorMessage));

            Assertions.assertThat(result).isEqualTo(ErrorMessages.BENUTZER_BEREITS_ANGEMELDET + errorMessage);
        }

        @Test
        void should_returnFixedErrorMessage_when_exceptionIsInstanceOfInternalAuthenticationServiceException() {
            val errorMessage = "bad credentials";
            val result = unitUnderTest.getErrorMessage(new InternalAuthenticationServiceException(errorMessage));
