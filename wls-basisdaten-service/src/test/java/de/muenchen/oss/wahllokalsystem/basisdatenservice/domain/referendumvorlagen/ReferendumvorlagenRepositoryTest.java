...
class ReferendumvorlagenRepositoryTest {
  ...
  @Autowired private ReferendumvorlageRepository referendumvorlageRepository;
    ...
    void should_removeReferendumvorlagenAndChildren_when_wahlIdMatches() {
      ...
      val referendumvorlagenToKeep =
          transactionTemplate.execute(
              status -> {
                referendumvorlagenRepository.save(
                ...
                return referendumvorlagenRepository.save(
                    createReferendumvorlagenWithBezirkAndWahlID("wahlbezirk1", "wahlIDToKeep"));
              });

      transactionTemplate.executeWithoutResult(
          status -> referendumvorlagenRepository.deleteAllByBezirkUndWahlID_WahlID(wahlIDToDelete));

      Assertions.assertThat(referendumvorlagenRepository.count()).isEqualTo(1);
      Assertions.assertThat(
              referendumvorlagenRepository.existsById(referendumvorlagenToKeep.getId()))
          .isTrue();

      Assertions.assertThat(referendumvorlageRepository.count())
          .isEqualTo(referendumvorlagenToKeep.getReferendumvorlagen().size());
      referendumvorlagenToKeep
          .getReferendumvorlagen()
          .forEach(
              referendumvorlage ->
                  Assertions.assertThat(
                          referendumvorlageRepository.existsById(referendumvorlage.getId()))
                      .isTrue());
    }

    private Referendumvorlagen createReferendumvorlagenWithBezirkAndWahlID(
      ...
      val referendumvorlage1 =
          new Referendumvorlage(
              null, null, UUID.randomUUID().toString(), 1L, "", "", new HashSet<>());
