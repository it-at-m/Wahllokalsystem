...
class WahlRepositoryTest {
  ...
  @Autowired private WahlRepository repository;
  ...
  void should_returnWahlenList_when_findByWahltagOrderByReihenfolgeIsCalled() {
    ...
    val expectedWahlen =
        wahlenToSave.stream()
            .filter(wahl -> Objects.equals(wahl.getWahltag(), wahltagToFind))
            .sorted(Comparator.comparing(Wahl::getReihenfolge))
