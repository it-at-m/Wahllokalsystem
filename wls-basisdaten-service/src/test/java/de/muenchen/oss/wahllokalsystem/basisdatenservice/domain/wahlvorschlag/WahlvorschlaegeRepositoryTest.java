...
class WahlvorschlaegeRepositoryTest {
  ...
  @Autowired private WahlvorschlaegeRepository wahlvorschlaegeRepository;

  @Autowired private WahlvorschlagRepository wahlvorschlagRepository;

  @Autowired private KandidatRepository kandidatRepository;
  ...
  class DeleteAllByBezirkUndWahlID_WahlID {
    ...
    void should_removeWahlvorschlaegeAndChildren_when_wahlIdMatches() {
              ...
              status -> {
                wahlvorschlaegeRepository.save(
                    createWahlvorschlaegeWithKandidatenWithBezirkAndWahlID(
                        "wahlbezirk1", wahlIDToDelete));
                wahlvorschlaegeRepository.save(
                    createWahlvorschlaegeWithKandidatenWithBezirkAndWahlID(
                        "wahlbezirk2", wahlIDToDelete));

                return wahlvorschlaegeRepository.save(
                    createWahlvorschlaegeWithKandidatenWithBezirkAndWahlID(
                        "wahlbezirk1", "wahlIDToKeep"));
              });
      ...
      Assertions.assertThat(wahlvorschlaegeRepository.existsById(wahlvorschlaegeToKeep.getId()))
          .isTrue();

      Assertions.assertThat(wahlvorschlagRepository.count())
          .isEqualTo(wahlvorschlaegeToKeep.getWahlvorschlaege().size());
      wahlvorschlaegeToKeep
          .getWahlvorschlaege()
          .forEach(
              wahlvorschlag ->
                  Assertions.assertThat(wahlvorschlagRepository.existsById(wahlvorschlag.getId()))
                      .isTrue());
      ...
      wahlvorschlaegeToKeep.getWahlvorschlaege().stream()
          ...
          .forEach(
              kandidat ->
                  Assertions.assertThat(kandidatRepository.existsById(kandidat.getId())).isTrue());
    }

    private Wahlvorschlaege createWahlvorschlaegeWithKandidatenWithBezirkAndWahlID(
