...
import de.muenchen.oss.wahllokalsystem.basisdatenservice.service.wahltag.WahltagModelMapper;
...
public class WahltageControllerIntegrationTest {
  ...
  @Autowired WahltageDTOMapper dtoMapper;

  @Autowired WahltagModelMapper modelMapper;

  @Autowired WahltageClientMapper wahltageClientMapper;
  ...
  class GetWahltage {
    ...
    void should_returnWahltagDTOList_when_loadedFromExternal() throws Exception {
      ...
      val expectedResponseBody =
          dtoMapper.fromListOfWahltagModelToListOfWahltagDTO(
              wahltageClientMapper.fromRemoteClientSetOfWahltagDTOtoListOfWahltagModel(
                  eaiWahltage));
    ...
    void should_returnWahltagList_when_externalDataIsPersisted() throws Exception {
      ...
      val expectedListOfData =
          modelMapper.fromWahltagModelToWahltagEntityList(
              wahltageClientMapper.fromRemoteClientSetOfWahltagDTOtoListOfWahltagModel(
                  eaiWahltage));
    ...
    @Test
    void should_returnWahltagDTOList_when_loadedFromRemoteFirstAndUpdatedRepositoryAfter()
        throws Exception {
      val entitiesToFindInRepository =
          modelMapper.fromWahltagModelToWahltagEntityList(
              wahltageClientMapper.fromRemoteClientSetOfWahltagDTOtoListOfWahltagModel(
                  createClientWahltageDTO(true)));
      ...
      val expectedResponseBody_1 =
          dtoMapper.fromListOfWahltagModelToListOfWahltagDTO(
              modelMapper.fromWahltagEntityToWahltagModelList(
                  (List<Wahltag>) savedEntitiesInRepository_1));
  ...
  private Set<de.muenchen.oss.wahllokalsystem.basisdatenservice.eai.aou.model.WahltagDTO>
      createClientWahltageDTO(boolean beschreibungPartPreNumber) {
    ...
    wahltag1.setBeschreibung(
        (beschreibungPartPreNumber) ? "diff_beschreibungWahltag1" : "beschreibungWahltag1");
    ...
    wahltag2.setBeschreibung(
        (beschreibungPartPreNumber) ? "diff_beschreibungWahltag2" : "beschreibungWahltag2");
    ...
    wahltag3.setBeschreibung(
        (beschreibungPartPreNumber) ? "diff_beschreibungWahltag3" : "beschreibungWahltag3");
