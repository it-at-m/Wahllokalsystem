...
public class WahltermindatenControllerIntegrationTest {
  ...
  @MockitoSpyBean WahlvorschlaegeRepository wahlvorschlaegeRepository;
  @Autowired WahlvorschlagRepository wahlvorschlagRepository;
  @Autowired KandidatRepository kandidatRepository;

  @MockitoSpyBean ReferendumvorlagenRepository referendumvorlagenRepository;
  @Autowired ReferendumvorlageRepository referendumvorlageRepository;

  @MockitoSpyBean WahlRepository wahlRepository;
  @MockitoSpyBean WahlbezirkRepository wahlbezirkRepository;
  @MockitoSpyBean KopfdatenRepository kopfdatenRepository;

  @MockitoSpyBean AsyncWahltermindatenService asyncWahltermindatenService;

  @Autowired private WahltagRepository wahltagRepository;

  @Autowired WahldatenClientMapper wahldatenClientMapper;
  ...
  class DeleteWahltermindaten {
    ...
    void setup() throws Exception {
      ...
      wahltagForDelete =
          wahltagRepository.save(
              new Wahltag("wahltagForDelete", LocalDate.parse(dateStringForDelete), null, "12"));
      ...
      wahlbezirkeToDelete =
          createWahlbezirkeToDelete(
              localDateForDelete, wahlToDelete1.getWahlID(), wahlToDelete2.getWahlID());
    ...
    @Test
    void should_deleteWahlenAndWahlbezirkeOnWahlenOfWahltag_when_wahltagIDIsGiven()
        throws Exception {
      ...
      val wahlbezirkToKeep =
          wahlbezirkRepository.save(
              new Wahlbezirk(
                  ...
                  WahlbezirkArt.UWB,
                  "1",
                  wahlToKeep.getWahlID()));
      val wahlvorschlaegeToKeep =
          createWahlvorschlaege(wahlToKeep.getWahlID(), wahlbezirkToKeep.getWahlbezirkID(), "3");
      val referendumvorlagenToKeep =
          createReferendunvorlagen(wahlToKeep.getWahlID(), wahlbezirkToKeep.getWahlbezirkID(), "3");
      val kopfdatenToKeep =
          kopfdatenRepository.save(
              new Kopfdaten(
                  new BezirkUndWahlID(wahlToKeep.getWahlID(), wahlbezirkToKeep.getWahlbezirkID()),
                  "LHM",
                  Stimmzettelgebietsart.WK,
                  "3",
                  "SHZ to keep",
                  wahlToKeep.getName(),
      ...
      val request =
          MockMvcRequestBuilders.delete(
              "/businessActions/wahltermindaten/" + wahltagForDelete.getWahltagID());
      mockMvc.perform(request).andExpect(status().isOk());

      // verify data to keep are not deleted
      Assertions.assertThat(wahlRepository.findById(wahlToKeep.getWahlID())).isPresent();
      Assertions.assertThat(wahlbezirkRepository.findById(wahlbezirkToKeep.getWahlbezirkID()))
          .isPresent();
      Assertions.assertThat(wahlvorschlaegeRepository.findById(wahlvorschlaegeToKeep.getId()))
          .isPresent();
      Assertions.assertThat(referendumvorlagenRepository.findById(referendumvorlagenToKeep.getId()))
          .isPresent();
      Assertions.assertThat(kopfdatenRepository.findById(kopfdatenToKeep.getBezirkUndWahlID()))
          .isPresent();

      // verify no additional data exists
      val expectedCountWahlvorschlaeg = wahlvorschlaegeToKeep.getWahlvorschlaege().size();
      val expectedCountKandidaten =
          (Long)
              wahlvorschlaegeToKeep.getWahlvorschlaege().stream()
      ...
      Assertions.assertThat(referendumvorlageRepository.count())
          .isEqualTo(expectedCountReferendumvorlage);
    ...
    void should_keepAllExistingData_when_deleteOfWahlbezirkeFailed() throws Exception {
      Mockito.doThrow(new RuntimeException("test transactional"))
          .when(wahlbezirkRepository)
          .deleteByWahltag(any());
      ...
      verifyNoDataForDeletionIsDeleted(
          wahlvorschlaegeToDelete,
    ...
    void should_keepAllExistingData_when_deleteOfKopfdatenFailed() throws Exception {
      Mockito.doThrow(new RuntimeException("test transactional"))
          .when(kopfdatenRepository)
          .deleteAllByBezirkUndWahlID_WahlID(any());
    ...
    void should_keepAllExistingData_when_deleteOfWahlvorschlaegeFailed() throws Exception {
      Mockito.doThrow(new RuntimeException("test transactional"))
          .when(wahlvorschlaegeRepository)
          .deleteAllByBezirkUndWahlID_WahlID(any());
    ...
    void should_keepAllExistingData_when_deleteOfReferendumvorlagenFailed() throws Exception {
      Mockito.doThrow(new RuntimeException("test transactional"))
          .when(referendumvorlagenRepository)
          .deleteAllByBezirkUndWahlID_WahlID(any());
    ...
    private void verifyNoDataForDeletionIsDeleted(
        Collection<Wahlvorschlaege> wahlvorschlaegeToDelete,
        Collection<Referendumvorlagen> referendumvorlagenToDelete,
        Collection<Wahlbezirk> wahlbezirkeToDelete,
        Collection<Kopfdaten> kopfdatenToDelete) {
      val expectedCountWahlvorschlaeg =
          wahlvorschlaegeToDelete.stream()
              .map(wahlvorschlaege -> wahlvorschlaege.getWahlvorschlaege().size())
              .mapToLong(Integer::longValue)
              .sum();
      val expectedCountKandidaten =
          wahlvorschlaegeToDelete.stream()
              ...
              .sum();
      val expectedCountReferendumvorlage =
          referendumvorlagenToDelete.stream()
      ...
      Assertions.assertThat(wahlvorschlaegeRepository.count())
          .isEqualTo(wahlvorschlaegeToDelete.size());
      Assertions.assertThat(wahlvorschlagRepository.count()).isEqualTo(expectedCountWahlvorschlaeg);
      Assertions.assertThat(kandidatRepository.count()).isEqualTo(expectedCountKandidaten);
      Assertions.assertThat(referendumvorlagenRepository.count())
          .isEqualTo(referendumvorlagenToDelete.size());
      Assertions.assertThat(referendumvorlageRepository.count())
          .isEqualTo(expectedCountReferendumvorlage);
    ...
    private void setupWireMockForWahlClient(String wahlToDeleteID1, String wahlToDeleteID2)
      ...
      val wahlclientResponse =
          new BasisdatenDTO()
              .basisstrukturdaten(
                  Set.of(
                      new BasisstrukturdatenDTO().wahlID(wahlToDeleteID1),
                      new BasisstrukturdatenDTO().wahlID(wahlToDeleteID2)));
      WireMock.stubFor(
    ...
    private Collection<Wahlbezirk> createWahlbezirkeToDelete(
        final LocalDate wahltagDate, final String wahlToDeleteID1, final String wahlToDeleteID2) {
      val wahlbezirk1ToDeleteWahl1 =
          wahlbezirkRepository.save(
              new Wahlbezirk("wbz1_1", wahltagDate, "1", WahlbezirkArt.UWB, "1", wahlToDeleteID1));
      val wahlbezirk2ToDeleteWahl1 =
          wahlbezirkRepository.save(
              new Wahlbezirk("wbz1_2", wahltagDate, "2", WahlbezirkArt.UWB, "1", wahlToDeleteID1));
      val wahlbezirk1ToDeleteWahl2 =
          wahlbezirkRepository.save(
              new Wahlbezirk("wbz2_1", wahltagDate, "1", WahlbezirkArt.UWB, "2", wahlToDeleteID2));
      val wahlbezirk2ToDeleteWahl2 =
          wahlbezirkRepository.save(
              new Wahlbezirk("wbz2_2", wahltagDate, "2", WahlbezirkArt.UWB, "2", wahlToDeleteID2));

      return List.of(
          wahlbezirk1ToDeleteWahl1,
          wahlbezirk2ToDeleteWahl1,
          wahlbezirk1ToDeleteWahl2,
          wahlbezirk2ToDeleteWahl2);
    }

    private Collection<Kopfdaten> createKopfdatenToDelete(
        final Collection<Wahlbezirk> wahlbezirke) {
      return wahlbezirke.stream()
          .map(
              wahlbezirk ->
                  kopfdatenRepository.save(
                      new Kopfdaten(
                          new BezirkUndWahlID(wahlbezirk.getWahlID(), wahlbezirk.getWahlbezirkID()),
                          "LHM",
                          Stimmzettelgebietsart.WK,
                          "1",
                          "SGZ 1_1",
    ...
    private Collection<Wahlvorschlaege> createWahlvorschlaegeToDelete(
        final Collection<Wahlbezirk> wahlbezirke) {
      return wahlbezirke.stream()
          .map(
              wahlbezirk ->
                  createWahlvorschlaege(
                      wahlbezirk.getWahlID(), wahlbezirk.getWahlbezirkID(), wahlbezirk.getNummer()))
          .toList();
    }

    private Collection<Referendumvorlagen> createReferendumvorlagenToDelete(
        final Collection<Wahlbezirk> wahlbezirke) {
      return wahlbezirke.stream()
          .map(
              wahlbezirk ->
                  createReferendunvorlagen(
                      wahlbezirk.getWahlID(), wahlbezirk.getWahlbezirkID(), wahlbezirk.getNummer()))
          .toList();
    }

    private Wahlvorschlaege createWahlvorschlaege(
    ...
    private Referendumvorlagen createReferendunvorlagen(
        final String wahlID, final String wahlbezirkID, final String stimmzettelgebietID) {
    ...
    void should_persistImportedData_when_wahltagIDIsValidWahltag() throws Exception {
      ...
      val request =
          MockMvcRequestBuilders.put(
              "/businessActions/wahltermindaten/" + wahltagToGetWahltermindaten.getWahltagID());
      mockMvc.perform(request).andExpect(status().isOk());
      ...
      val expectedBasisdatenModel =
          wahldatenClientMapper.fromRemoteClientDTOToModel(basisstrukturdatenToImport);
      Mockito.verify(asyncWahltermindatenService)
          .initVorlagenAndVorschlaege(
              eq(
                  new WahltagWithNummerModel(
                      wahltagToGetWahltermindaten.getWahltag(),
                      wahltagToGetWahltermindaten.getNummer())),
          ...
          .basisstrukturdaten(
              Set.of(
                  new BasisstrukturdatenDTO()
                      .wahlID("wahlID1")
                      .wahltag(localDateOfWahltag)
                      .wahlbezirkID("wahlbezirkID1_1")
                      .stimmzettelgebietID("sgzID1"),
                  new BasisstrukturdatenDTO()
                      .wahlID("wahlID1")
                      .wahltag(localDateOfWahltag)
                      .wahlbezirkID("wahlbezirkID1_2")
                      .stimmzettelgebietID("sgzID1"),
                  new BasisstrukturdatenDTO()
                      .wahlID("wahlID1")
                      .wahltag(localDateOfWahltag)
                      .wahlbezirkID("wahlbezirkID1_3")
                      .stimmzettelgebietID("sgzID1"),
                  new BasisstrukturdatenDTO()
                      .wahlID("wahlID2")
                      .wahltag(localDateOfWahltag)
                      .wahlbezirkID("wahlbezirkID2_1")
                      .stimmzettelgebietID("sgzID2"),
                  new BasisstrukturdatenDTO()
                      .wahlID("wahlID2")
                      .wahltag(localDateOfWahltag)
                      .wahlbezirkID("wahlbezirkID2_2")
                      .stimmzettelgebietID("sgzID2")))
          .stimmzettelgebiete(
              Set.of(
          ...
          .wahlen(
              Set.of(
                  new WahlDTO()
                      .name("wahl 1")
                      .wahltag(localDateOfWahltag)
                      .identifikator("wahlID1")
                      .wahlart(WahlDTO.WahlartEnum.BTW)
                      .nummer("1"),
                  new WahlDTO()
                      .name("wahl 2")
                      .wahltag(localDateOfWahltag)
                      .identifikator("wahlID2")
                      .wahlart(WahlDTO.WahlartEnum.BTW)
                      .nummer("2")))
          .wahlbezirke(
              Set.of(
  ...
  private void setupWireMockForWahltagClient(Wahltag wahltag) throws JsonProcessingException {
    val wahltagClientResponse =
        Set.of(
            new WahltagDTO()
                .identifikator(wahltag.getWahltagID())
                .tag(wahltag.getWahltag())
