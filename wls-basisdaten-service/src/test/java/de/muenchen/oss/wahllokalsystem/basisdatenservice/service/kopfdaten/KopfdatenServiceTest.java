...
class KopfdatenServiceTest {
  ...
  @Mock KopfdatenValidator kopfdatenValidator;

  @Mock KopfdatenMapper kopfdatenMapper;

  @Mock WahldatenClient wahldatenClient;

  @Mock KonfigurierterWahltagClient konfigurierterWahltagClient;

  @InjectMocks KopfdatenService unitUnderTest;
  ...
  class GetKopfdaten {
    ...
    void should_loadDataFromRemote_when_notExistingInRepo() {
      ...
      val mockedKonfigurierterWahltagFromClient =
          MockDataFactory.createClientKonfigurierterWahltagModel(forDate, true);
      val mockedBasisdatenModelFromClient =
          MockDataFactory.createBasisdatenModel(mockedKonfigurierterWahltagFromClient.wahltag());
      val mockedKopfdatenModelByInitializer =
          MockDataFactory.createKopfdatenModelFor(wahlID, wahlbezrkID);
      val mockedKopfdatenModelMappedToEntity =
          MockDataFactory.createKopfdatenEntityFor(wahlID, wahlbezrkID);

      Mockito.when(kopfdatenRepository.findById(bezirkUndWahlId)).thenReturn(Optional.empty());
      Mockito.when(kopfdatenModelMapper.toEntity(mockedKopfdatenModelByInitializer))
          .thenReturn(mockedKopfdatenModelMappedToEntity);
      ...
      Mockito.when(
              wahldatenClient.loadBasisdaten(
                  new WahltagWithNummerModel(
                      mockedKonfigurierterWahltagFromClient.wahltag(),
                      mockedKonfigurierterWahltagFromClient.nummer())))
          .thenReturn(mockedBasisdatenModelFromClient);
      Mockito.when(
              kopfdatenMapper.initKopfdata(wahlID, wahlbezrkID, mockedBasisdatenModelFromClient))
          .thenReturn(mockedKopfdatenModelByInitializer);
    ...
    void should_loadDataFromRepo_when_dataIsPresentAndNotFromRemoteClient() {
      ...
      Mockito.when(kopfdatenModelMapper.toModel(kopfdatenEntityInRepo))
          .thenReturn(expectedKopfdaten);
