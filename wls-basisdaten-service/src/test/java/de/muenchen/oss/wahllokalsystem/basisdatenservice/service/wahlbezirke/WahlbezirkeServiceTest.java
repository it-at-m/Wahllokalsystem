package de.muenchen.oss.wahllokalsystem.basisdatenservice.service.wahlbezirke;
...
class WahlbezirkeServiceTest {
  ...
  @InjectMocks WahlbezirkeService unitUnderTest;
  ...
  class GetWahlbezirke {
    ...
    void should_loadDataFromRemote_when_notExistingInRepo() {
      ...
      val mockedwahlbezirkeModelFromClient =
          MockDataFactory.createSetOfWahlbezirkModel("", wahltag.wahltag());
      val wahlen = MockDataFactory.createWahlModelList("", wahltagDate);
      val wahlModels =
          MockDataFactory.createWahlModelList("", wahltag.wahltag()).stream()
              .filter((wahl) -> wahl.wahltag().equals(wahltag.wahltag()))
              .toList();
      val mergedWahlbezirkeWithWahlen =
          mergeWahlbezirkeWithWahlen(mockedwahlbezirkeModelFromClient, wahlModels);
      val mergedWahlbezirkEntitiesFromRepo =
          buildWahlbezirEntitiesFromModels(mergedWahlbezirkeWithWahlen);
      ...
      Mockito.when(wahlenService.getExistingWahlenOrderedByReihenfolge(wahltagID))
          .thenReturn(wahlen);
      Mockito.when(wahlbezirkRepository.findByWahltag(wahltag.wahltag()))
          .thenReturn(mergedWahlbezirkEntitiesFromRepo);
      Mockito.when(
              wahlbezirkModelMapper.fromListOfWahlbezirkEntityToListOfWahlbezirkModel(
                  mergedWahlbezirkEntitiesFromRepo))
    ...
    void should_loadDataFromRepo_when_presentAndNotFromRemoteClient() {
      ...
      val mockedwahlbezirkeModelFromClient =
          MockDataFactory.createSetOfWahlbezirkModel("", wahltag.wahltag());
      val notExpectedMockedwahlbezirkeModelFromClient =
          MockDataFactory.createSetOfWahlbezirkModel("NotExpected", wahltag.wahltag());
      val wahlModels =
          MockDataFactory.createWahlModelList("", wahltag.wahltag()).stream()
              .filter((wahl) -> wahl.wahltag().equals(wahltag.wahltag()))
              .toList();
      val notExpectedWahlModels =
          MockDataFactory.createWahlModelList("NotExpected", wahltag.wahltag()).stream()
              .filter((wahl) -> wahl.wahltag().equals(wahltag.wahltag()))
              .toList();
      val mergedWahlbezirkeWithWahlen =
          mergeWahlbezirkeWithWahlen(mockedwahlbezirkeModelFromClient, wahlModels);
      ...
      val notExpectedMergedWahlbezirkeWithWahlen =
          mergeWahlbezirkeWithWahlen(
              notExpectedMockedwahlbezirkeModelFromClient, notExpectedWahlModels);
  ...
  private List<Wahlbezirk> buildWahlbezirEntitiesFromModels(
      List<WahlbezirkModel> remoteSetWahlbezirkeModels) {
    List<Wahlbezirk> remoteList = new ArrayList<>();
    remoteSetWahlbezirkeModels.forEach(
        (wbModel) ->
            remoteList.add(
                new Wahlbezirk(
                    wbModel.wahlbezirkID(),
                    wbModel.wahltag(),
                    wbModel.nummer(),
                    WahlbezirkArt.valueOf(wbModel.wahlbezirkart().name()),
                    wbModel.wahlnummer(),
                    wbModel.wahlID())));
    return remoteList;
  }

  private List<WahlbezirkModel> mergeWahlbezirkeWithWahlen(
      Set<WahlbezirkModel> remoteSetWahlbezirkeModels, List<WahlModel> wahlModels) {
    List<Wahlbezirk> responselist = new ArrayList<>();
    List<Wahlbezirk> remoteList =
        buildWahlbezirEntitiesFromModels(remoteSetWahlbezirkeModels.stream().toList());
    remoteList.forEach(
        wahlbezirk -> {
          if (null != wahlModels) {
            wahlModels.stream()
