package de.muenchen.oss.wahllokalsystem.basisdatenservice.service.wahltag;
...
class WahltageServiceTest {
  ...
  @Mock WahltagRepository wahltagRepository;
  @Mock WahltagModelMapper wahltagModelMapper;
  ...
  class GetWahltage {
    ...
    void should_loadDataFromRemote_when_existingInRepoAndRepoIsUpdated() {
      ...
      Mockito.when(wahltageClient.getWahltage(LocalDate.now().minusMonths(3)))
          .thenReturn(mockedClientResponse);
      Mockito.when(wahltagModelMapper.fromWahltagModelToWahltagEntityList(mockedClientResponse))
          .thenReturn(mockedListOfEntities);
      Mockito.when(wahltagRepository.findAllByOrderByWahltagAsc()).thenReturn(mockedListOfEntities);
      Mockito.when(wahltagModelMapper.fromWahltagEntityToWahltagModelList(mockedListOfEntities))
          .thenReturn(mockedMappedSavedEntities);
  ...
  class GetWahltagByID {
    ...
    void should_returnWahltag_when_callingGetById() {
      ...
      Mockito.when(wahltagRepository.findById(wahltagID))
          .thenReturn(Optional.of(mockedRepoResponse));
      Mockito.when(wahltagModelMapper.toModel(mockedRepoResponse))
          .thenReturn(mockedMappedRepoResponse);

      Assertions.assertThat(unitUnderTest.getWahltagByID(wahltagID))
    ...
    void should_throwException_when_notFoundInRepo() {
      ...
      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.getWahltagByID(wahltagID))
