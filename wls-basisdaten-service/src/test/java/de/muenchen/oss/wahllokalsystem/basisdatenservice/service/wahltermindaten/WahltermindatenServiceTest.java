...
class WahltermindatenServiceTest {
  ...
  @Mock WahltermindatenValidator wahltermindatenValidator;

  @Mock WahltageService wahltageService;

  @Mock WahldatenClient wahldatenClient;

  @Mock WahlModelMapper wahlModelMapper;

  @Mock WahlbezirkModelMapper wahlbezirkModelMapper;

  @Mock KopfdatenModelMapper kopfdatenModelMapper;

  @Mock WahlRepository wahlRepository;

  @Mock WahlbezirkRepository wahlbezirkRepository;

  @Mock KopfdatenRepository kopfdatenRepository;

  @Mock WahlvorschlaegeRepository wahlvorschlaegeRepository;

  @Mock ReferendumvorlagenRepository referendumvorlagenRepository;

  @Mock KopfdatenMapper kopfDataInitializer;

  @Mock ExceptionFactory exceptionFactory;

  @Mock AsyncWahltermindatenService asyncWahltermindatenService;

  @InjectMocks WahltermindatenService unitUnderTest;

  @Nested
  class PutWahltermindaten {

    @Captor ArgumentCaptor<Collection<Wahlbezirk>> wahlbezirkEntitiesCaptor;
    ...
    void should_throwException_when_wahltagIDIsInvalid() {
      ...
      val mockedValidationException =
          FachlicheWlsException.withCode("000").buildWithMessage("validation failed");

      Mockito.doThrow(mockedValidationException)
          .when(wahltermindatenValidator)
          .validateParameterToInitWahltermindaten(wahltagID);
    ...
    void should_throwException_when_wahltagWithIDDoesNotExist() {
      ...
      val mockedWahltageServiceResponse =
          List.of(new WahltagModel("otherWahltagID", LocalDate.now(), "", ""));
      ...
      Mockito.when(
              wahldatenClient.loadBasisdaten(
                  new WahltagWithNummerModel(
                      mockedMatchingWahltag.wahltag(), mockedMatchingWahltag.nummer())))
          .thenReturn(null);
      Mockito.when(
              exceptionFactory.createFachlicheWlsException(
                  ExceptionConstants.GET_BASISDATEN_NO_DATA))
          .thenReturn(mockedWlsException);
    ...
    void should_persistData_when_gettingBasisdaten() {
      ...
      val mockedWahlenMappedAsEntity =
          List.of(createWahlWithNummer("1"), createWahlWithNummer("2"));
      val mockedWahlbezirkeMappedAsEntity =
          Set.of(createWahlbezirkWithWahlnummer("1"), createWahlbezirkWithWahlnummer("2"));
      val mockedKopfdatenListen =
          List.of(KopfdatenModel.builder().build(), KopfdatenModel.builder().build());
      ...
      Mockito.when(wahlModelMapper.fromListOfWahlModeltoListOfWahlEntities(any()))
          .thenReturn(mockedWahlenMappedAsEntity);
      Mockito.when(wahlbezirkModelMapper.fromListOfWahlbezirkModeltoListOfWahlbezirkEntities(any()))
          .thenReturn(mockedWahlbezirkeMappedAsEntity);
      Mockito.when(kopfDataInitializer.initKopfdaten(mockedWahldatenClientResponse))
          .thenReturn(mockedKopfdatenListen);
      ...
      Mockito.verify(kopfdatenRepository)
          .saveAll(List.of(mockedKopfdatenMappedAsEntity, mockedKopfdatenMappedAsEntity));

      Assertions.assertThat(wahlbezirkEntitiesCaptor.getAllValues().size()).isEqualTo(1);
      Assertions.assertThat(wahlbezirkEntitiesCaptor.getValue())
          .containsOnly(
    ...
    private static BasisdatenModel createMockedBasisdatenModel(LocalDate wahltagDate) {
      ...
      return new BasisdatenModel(
          ...
          mockedWahldatenWahlen,
          mockedWahldatenWahlbezirke,
          Collections.emptySet());
    }
  ...
  class DeleteWahltermindaten {
    ...
    void should_throwException_when_wahltagIDIsNotValid() {
      ...
      Mockito.doThrow(mockedValidationException)
          .when(wahltermindatenValidator)
          .validateParameterToDeleteWahltermindaten(wahltagID);

      Assertions.assertThatThrownBy(() -> unitUnderTest.deleteWahltermindaten(wahltagID))
          .isSameAs(mockedValidationException);
    }
    ...
    void should_throwException_when_wahltagOfIDDoesNotExist() {
      ...
      val mockedFachlicheWlsException =
          FachlicheWlsException.withCode("000").buildWithMessage("fachlicheWlsException");
    ...
    void should_deleteDataInReposOfWahlen_when_wahltagIdHasWahltagWithBasisdaten() {
      ...
      val mockedWahltageServiceResponse = List.of(mockedMatchingWahltag);
      val mockedBasisstrukturdatenModels =
          Set.of(
              createBasisstrukturdatenModelWithWahlID("wahlID1"),
              createBasisstrukturdatenModelWithWahlID("wahlID2"));
      val mockedWahldatenClientResponse =
          new BasisdatenModel(mockedBasisstrukturdatenModels, null, null, null);

      Mockito.when(wahltageService.getWahltage()).thenReturn(mockedWahltageServiceResponse);
      Mockito.when(
              wahldatenClient.loadBasisdaten(
                  new WahltagWithNummerModel(
                      mockedDataWahltagDate, mockedMatchingWahltag.nummer())))
          .thenReturn(mockedWahldatenClientResponse);
