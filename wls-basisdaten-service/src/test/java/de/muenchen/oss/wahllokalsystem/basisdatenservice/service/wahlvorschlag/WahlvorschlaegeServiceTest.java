...
class WahlvorschlaegeServiceTest {
  ...
  @Mock WahlvorschlaegeValidator wahlvorschlaegeValidator;
  @Mock WahlvorschlaegeClient wahlvorschlaegeClient;

  @InjectMocks WahlvorschlaegeService unitUnderTest;
  ...
  class GetWahlvorschlaege {
    ...
    void should_loadAndStoreData_when_serviceIsCalled() {
      ...
      mockedWahlvorschlagEntity1.setKandidaten(
          Set.of(mockedKandidatEntity1, mockedKandidatEntity2));
      ...
      mockedMappedEntity.setWahlvorschlaege(
          Set.of(mockedWahlvorschlagEntity1, mockedWahlvorschlagEntity2));

      val mockedMappedSavedEntity = WahlvorschlaegeModel.builder().build();

      Mockito.when(wahlvorschlaegeRepository.findByBezirkUndWahlID(wahlUndBezirkID))
          .thenReturn(Optional.empty());
      Mockito.when(wahlvorschlaegeRepository.save(mockedMappedEntity))
          .thenReturn(mockedMappedEntity);
      Mockito.when(wahlvorschlaegeClient.getWahlvorschlaege(wahlUndBezirkID))
          .thenReturn(mockedClientResponse);
      Mockito.when(wahlvorschlaegeModelMapper.toEntity(mockedClientResponse))
          .thenReturn(mockedMappedEntity);
      Mockito.when(wahlvorschlaegeModelMapper.toModel(mockedMappedEntity))
          .thenReturn(mockedMappedSavedEntity);
    ...
    void should_loadExistingData_when_serviceIsCalled() {
      ...
      Mockito.when(wahlvorschlaegeRepository.findByBezirkUndWahlID(wahlUndBezirkID))
          .thenReturn(Optional.of(mockedWahlvorschlaegeFromRepo));
      Mockito.when(wahlvorschlaegeModelMapper.toModel(mockedWahlvorschlaegeFromRepo))
          .thenReturn(mockedMappedWahlvorschlaegeAsModel);
    ...
    void should_throwRuntimeException_when_validationFailed() {
      ...
      Mockito.doThrow(mockedValidationException)
          .when(wahlvorschlaegeValidator)
          .validWahlIdUndWahlbezirkIDOrThrow(wahlUndBezirkID);

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.getWahlvorschlaege(wahlUndBezirkID))
    ...
    void should_throwRuntimeException_when_clientFailed() {
      ...
      Mockito.doThrow(mockedClientException)
          .when(wahlvorschlaegeClient)
          .getWahlvorschlaege(wahlUndBezirkID);

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.getWahlvorschlaege(wahlUndBezirkID))
          .isSameAs(mockedClientException);
    }
    ...
    void should_notPreventFromResultObject_when_exceptionIsPersisted() {
      ...
      Mockito.when(wahlvorschlaegeClient.getWahlvorschlaege(wahlUndBezirkID))
          .thenReturn(mockedWahlvorschlaegeModel);
      Mockito.when(wahlvorschlaegeModelMapper.toEntity(mockedWahlvorschlaegeModel))
          .thenReturn(mockedWahlvorschlaegeEntity);
      Mockito.doThrow(mockedPersistingException)
          .when(wahlvorschlaegeRepository)
          .save(mockedWahlvorschlaegeEntity);
