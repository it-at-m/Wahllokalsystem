...
class BeanstandeteWahlbriefeValidatorTest {
  ...
  @InjectMocks BeanstandeteWahlbriefeValidator unitUnderTest;
  ...
  class ValideReferenceOrThrow {
    ...
    void should_throwWlsException_when_wahlbezirkIDIsNull() {
      ...
      Mockito.when(
              exceptionFactory.createFachlicheWlsException(
                  ExceptionConstants.GETBEANSTANDETEWAHLBRIEFE_PARAMETER_UNVOLLSTAENDIG))
          .thenReturn(mockedWlsException);

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.valideReferenceOrThrow(invalidReference))
    ...
    void should_notThrowException_when_referenceIsValid() {
      ...
      org.junit.jupiter.api.Assertions.assertDoesNotThrow(
          () -> unitUnderTest.valideReferenceOrThrow(valideReference));
    }

    private BeanstandeteWahlbriefeReferenceModel.BeanstandeteWahlbriefeReferenceModelBuilder
        getValidReference() {
      return BeanstandeteWahlbriefeReferenceModel.builder()
          .wahlbezirkID("wbzId")
          .waehlerverzeichnisNummer(1L);
    }
  }
  ...
  class ValideModelOrThrow {
    ...
    void should_throwWlsException_when_beanstandeteWahlbriefeIsNull() {
      ...
      Mockito.when(
              exceptionFactory.createFachlicheWlsException(
                  ExceptionConstants.POSTBEANSTANDETEWAHLBRIEFE_PARAMETER_UNVOLLSTAENDIG))
          .thenReturn(mockedWlsException);

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.valideModelOrThrow(invalidModel))
          .isSameAs(mockedWlsException);
    }
    ...
    void should_notThrowException_when_modelIsValid() {
      ...
      org.junit.jupiter.api.Assertions.assertDoesNotThrow(
          () -> unitUnderTest.valideModelOrThrow(modelToValidate));
    }

    private BeanstandeteWahlbriefeModel.BeanstandeteWahlbriefeModelBuilder getValidModel() {
      return BeanstandeteWahlbriefeModel.builder()
          .beanstandeteWahlbriefe(new HashMap<>())
          .waehlerverzeichnisNummer(1L)
          .wahlbezirkID("wbzId");
    }
  }
