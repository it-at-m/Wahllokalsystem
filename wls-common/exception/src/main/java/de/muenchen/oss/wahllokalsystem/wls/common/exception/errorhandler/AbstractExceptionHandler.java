...
public abstract class AbstractExceptionHandler {
  ...
  private static final HttpStatus HTTP_STATUS_INFRASTRUKTURELLER_FEHLER =
    ...
    if (throwable instanceof WlsException wlsException) {
      ...
      data =
          new WlsExceptionDTO(
              ...
              getService(),
              String.format(
                  "Ursache: %s, Nachricht: %s", throwable.getClass(), throwable.getMessage()));
    }
  ...
  protected WlsExceptionDTO createForTransientException(final Throwable throwable) {
    return new WlsExceptionDTO(
        ...
        getService(),
        String.format(
            "Temporäres Problem, Ursache: %s, Nachricht: %s",
  ...
  protected ResponseEntity<WlsExceptionDTO> createResponse(final WlsExceptionDTO wlsExceptionDTO) {
    return switch (wlsExceptionDTO.category()) {
      case T ->
          new ResponseEntity<>(
              wlsExceptionDTO,
              ExceptionKonstanten.CODE_TRANSIENT.equals(wlsExceptionDTO.code())
                  ? HttpStatus.CONFLICT
                  : HTTP_STATUS_TECHNISCHER_FEHLER);
      case F ->
          ExceptionKonstanten.CODE_ENTITY_NOT_FOUND.equals(wlsExceptionDTO.code())
              ? new ResponseEntity<>(HttpStatus.NO_CONTENT)
              : new ResponseEntity<>(wlsExceptionDTO, HTTP_STATUS_FACHLICHER_FEHLER);
      case I -> new ResponseEntity<>(wlsExceptionDTO, HTTP_STATUS_INFRASTRUKTURELLER_FEHLER);
