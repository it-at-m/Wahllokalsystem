...
class FachlicheWlsExceptionTest {
  ...
  class WithCode {
    ...
    void should_returnFachlicheWlsExceptionWithAllPropertiesSet_when_buildWithAllStepsOfBuilder() {
      ...
      val fachlicheWlsException =
          FachlicheWlsException.withCode(code)
              .inService(serviceName)
              .withCause(causingException)
              .buildWithMessage(message);

      Assertions.assertThat(fachlicheWlsException.getCategory())
          .isSameAs(WlsExceptionCategory.FACHLICH);
