...
public class BezirkIDPermissionEvaluatorImpl implements BezirkIDPermissionEvaluator {
  ...
  private static final String AUTH_DETAILS_MAP_KEY_WAHLBEZIRKID_WAHLNUMMER =
  ...
  public boolean tokenUserBezirkIdMatches(String bezirkId, Authentication auth) {
    ...
    try {
      ...
      val bezirkIdMatches =
          (bezirkId != null)
              && (bezirkId.equals(bezirkIDFromToken)
                  || (wahlbezirkid_wahlnummer != null
                      && wahlbezirkid_wahlnummer.contains(bezirkId)));
      LOG.debug(
          "Check bezirkId {} from request against username {}, bezirkId {} from token or wahlbezirkid_wahlnummer {}. RESULT = {}",
          bezirkId,
          auth.getPrincipal(),
          bezirkIDFromToken,
          wahlbezirkid_wahlnummer,
