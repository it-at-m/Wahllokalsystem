...
class BezirkIDPermissionEvaluatorImplTest {
  ...
  private final BezirkIDPermissionEvaluatorImpl unitUnderTest =
      new BezirkIDPermissionEvaluatorImpl(List.of(authDetailRetriever));
  ...
  class TokenUserBezirkIdMatches {
    ...
    void should_returnFalseAndCreateLogWarning_when_authenticationIsNull() {
      ...
      Assertions.assertThat(loggerExtension.getFormattedMessages())
          .contains("No authentication object for bezirkId=1234");
    }
    ...
    void should_returnFalseAndCreateLogError_when_errorWhileCheckingOccurred() {
      ...
      Assertions.assertThat(loggerExtension.getFormattedMessages())
          .contains("Error while checking bezirkId.");
    }
    ...
    void should_returnTrue_when_bezirkIDMatchesFromToken() {
      ...
      Mockito.when(authDetailRetriever.getDetail("wahlbezirkID", auth))
          .thenReturn(Optional.of(wahlbezirkID));
      Mockito.when(authDetailRetriever.getDetail("wahlbezirkid_wahlnummer", auth))
    ...
    void should_returnTrue_when_bezirkIDMatchesFromWahlBezirkID() {
      ...
      Mockito.when(authDetailRetriever.getDetail("wahlbezirkID", auth))
          .thenReturn(Optional.empty());
      Mockito.when(authDetailRetriever.getDetail("wahlbezirkid_wahlnummer", auth))
          .thenReturn(Optional.of(wahlbezirkid_wahlnummer));
