...
public class SecurityUtils {
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(
            new UsernamePasswordAuthenticationToken(
                username, password, AuthorityUtils.createAuthorityList(authorities)));
  }
   ...
   * The first argument is the array with the subset of authorities and the second argument is the
   * missing authority <div>Example:</div> allRequiredAuthorities = { "auth1", "auth2", "auth3" }
   * <br>
   * Result in following arguments:<br>
   *
   * <ol>
   *   <li>Arguments.of({"auth2", "auth3"}, "auth1")
   *   <li>Arguments.of({"auth1", "auth3"}, "auth2")
   *   <li>Arguments.of({"auth1", "auth2"}, "auth3")
   * </ol>
   *
   ...
   *     missing. First argument is the subset, the second argument is the missing authority
   */
  public static Stream<Arguments> buildArgumentsForMissingAuthoritiesVariations(
      final String[] allRequiredAuthorities) {
    return Arrays.stream(allRequiredAuthorities)
        .map(
            authorityToRemove ->
                // remove one authority from all required authorities
                Arguments.of(
                    Arrays.stream(allRequiredAuthorities)
