...
public class NestedTestsuitesHaveMatchingMethodCondition extends ArchCondition<JavaClass> {
  ...
  public NestedTestsuitesHaveMatchingMethodCondition() {
    ...
    this.classNameEndingExclusions =
        Set.of("IntegrationTest", "ConfigurationTest", "ServiceSecurityTest", "ArchUnitTest");
  }
  ...
  public static final ArchCondition<JavaClass>
      haveMatchingPublicMethodNameIfTheyAreHighestNestedClass =
          new NestedTestsuitesHaveMatchingMethodCondition();
    ...
    if (topNestedClass.equals(classWithNestedAnnotation)) {
      if (topEnclosingClass.isPresent()) {
        try {
          ...
          val testedClassFullName =
              testSuiteClassFullName.substring(0, testSuiteClassFullName.lastIndexOf("Test"));
          ...
          val testedClassHasPublicMethodMatchingNestedClassName =
              Arrays.stream(testedClass.getMethods())
                  .anyMatch(method -> method.getName().equals(expectedMethodName));

          if (!testedClassHasPublicMethodMatchingNestedClassName) {
            events.add(
                SimpleConditionEvent.violated(
                    classWithNestedAnnotation,
                    "tested class \""
                        + testSuiteClassFullName
                        + "\" has no public method matching the nested class name: "
                        + expectedMethodName));
          }
          ...
          events.add(
              SimpleConditionEvent.violated(
                  classWithNestedAnnotation,
                  "expected class related to test suite not found: "
                      + topEnclosingClass.get().getFullName()));
        }
      } else {
        events.add(
            SimpleConditionEvent.violated(
                classWithNestedAnnotation, "nested annotation is not on an inner class"));
      }
    }
  }
