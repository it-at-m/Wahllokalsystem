...
class SecurityUtilsTest {
  ...
  class RunAs {
    ...
    void should_setUserAndPasswortInSecurityContext_when_called() {
      ...
      Assertions.assertThat(currentSecurityContext.getAuthentication().getPrincipal())
          .isEqualTo(username);
      Assertions.assertThat(currentSecurityContext.getAuthentication().getCredentials())
          .isEqualTo(password);
    }
    ...
    void should_setAuthoritiesInSecurityContext_when_called() {
      ...
      final Collection<SimpleGrantedAuthority> currentAuthorities =
          (Collection<SimpleGrantedAuthority>)
              SecurityContextHolder.getContext().getAuthentication().getAuthorities();

      final SimpleGrantedAuthority[] expectedGrantedAuthorities = {
    ...
    void should_throwIllegalArgumentException_when_authoritiyIsNull() {
      ...
      Assertions.assertThatThrownBy(() -> SecurityUtils.runAs("", "", nullAuthority))
    ...
    void should_throwIllegalArgumentException_when_authorityIsBlankString() {
      ...
      Assertions.assertThatThrownBy(() -> SecurityUtils.runAs("", "", blankAuthority))
          .isInstanceOf(IllegalArgumentException.class);
    }
  }
  ...
  class RunWith {
    ...
    void should_setDefaultUserInSecurityContext_when_called() {
      ...
      Assertions.assertThat(currentSecurityContext.getAuthentication().getPrincipal())
          .isEqualTo(SecurityUtils.TESTUSER_DEFAULT);
      Assertions.assertThat(currentSecurityContext.getAuthentication().getCredentials())
          .isEqualTo(SecurityUtils.TESTUSER_PASSWORD_DEFAULT);
    }
    ...
    void should_throwIllegalArgumentException_when_authorityIsNull() {
      final String nullAuthority = null;
      Assertions.assertThatThrownBy(() -> SecurityUtils.runWith(nullAuthority))
          .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void should_throwIllegalArgumentException_when_authorityIsBlank() {
      final String blankAuthority = "   ";
      Assertions.assertThatThrownBy(() -> SecurityUtils.runWith(blankAuthority))
          .isInstanceOf(IllegalArgumentException.class);
    }
  }
  ...
  class BuildArgumentsForMissingAuthoritiesVariations {
    ...
    @Test
    void
        should_createStreamWithCombinationsOfAuthoritiesWithOneAuthorityMissingInEachCombination_when_authoritiesAreGiven() {
      val authority1 = "authority1";
      val authority2 = "authority2";
      val authority3 = "authority3";

      val argumentList =
          SecurityUtils.buildArgumentsForMissingAuthoritiesVariations(
                  new String[] {authority1, authority2, authority3})
              .toList();

      Assertions.assertThat(argumentList).hasSize(3);
      Assertions.assertThat(argumentList.get(0).get())
          .isEqualTo(new Object[] {new String[] {authority2, authority3}, authority1});
      Assertions.assertThat(argumentList)
          .map(Arguments::get)
          .contains(
              createExpectedArgumentBody(authority1, authority2, authority3),
              createExpectedArgumentBody(authority2, authority1, authority3),
