...
public class MissingRequestParameterExceptionDataWrapperMapper {
  ...
  private static final String WAHLDATEN_SERVLET_PATH =
      WahldatenController.WAHLDATEN_REQUEST_MAPPING;

  private static final String PATTERN_ANY_OTHER_CHAR = ".*";

  final Map<String, Map<Pattern, ExceptionDataWrapper>> mapMissingRequestParameters =
      new HashMap<>();

  {
    addExceptionDataWrapper(
        "includingSince",
        WAHLDATEN_SERVLET_PATH + "/wahltage",
        ExceptionConstants.LOADWAHLTAGE_TAG_FEHLT);
    addExceptionDataWrapper(
        "forDate",
        WAHLDATEN_SERVLET_PATH + "/wahlbezirk",
    ...
    addExceptionDataWrapper(
        "forDate", WAHLDATEN_SERVLET_PATH + "/wahlen", ExceptionConstants.LOADWAHLEN_WAHLTAG_FEHLT);
    addExceptionDataWrapper(
        "withNummer",
        WAHLDATEN_SERVLET_PATH + "/wahlen",
        ExceptionConstants.LOADWAHLEN_NUMMER_FEHLT);
    addExceptionDataWrapper(
        "forDate",
        WAHLDATEN_SERVLET_PATH + "/basisdaten",
  ...
  public Optional<ExceptionDataWrapper> getExceptionDataWrapperForMissingRequestParameterByName(
      final String servletPath, final String requestParameterName) {
    ...
    val patternMapping =
        mappingsForParameter.keySet().stream()
            .filter(pattern -> pattern.matcher(servletPath).matches())
            .findFirst();
    return patternMapping.map(mappingsForParameter::get);
  }

  private void addExceptionDataWrapper(
