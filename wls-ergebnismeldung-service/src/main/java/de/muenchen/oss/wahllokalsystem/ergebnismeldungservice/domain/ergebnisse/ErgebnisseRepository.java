...
public interface ErgebnisseRepository extends CrudRepository<Ergebnisse, BezirkUndWahlIDStapelart> {
  ...
  @Query(
      "SELECT e FROM Ergebnisse e WHERE e.bezirkUndWahlIDStapelart.wahlbezirkID = (:wahlbezirkID) AND e.bezirkUndWahlIDStapelart.wahlID = (:wahlID)")
  List<Ergebnisse> findByWahlbezirkIDAndWahlD(
      @Param("wahlbezirkID") String wahlbezirkID, @Param("wahlID") String wahlID);
}
