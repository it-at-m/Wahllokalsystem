...
public class ErgebnismeldungService {
  ...
  private static final Logger SYSLOGGER =
      LoggerFactory.getLogger("ERGEBNISMELDUNG_SERVICE_SIEM_LOGGER");
  ...
  @PreAuthorize(
      "hasAuthority('Ergebnismeldung_BUSINESSACTION_SendErgebnisse')"
          + "and @bezirkIdPermissionEvaluator.tokenUserBezirkIdMatches(#criteria.wahlbezirkID, authentication)")
  public boolean sendErgebnisse(
      @P("criteria") final ErgebnisseToSendCriteriaModel ergebnisseToSendCriteria) {
    ...
    assertWahlIsGeschlossen(
        new BezirkUndWahlID(
            ergebnisseToSendCriteria.wahlID(), ergebnisseToSendCriteria.wahlbezirkID()));
  ...
  private void sendErgebnisseToEAI(final ErgebnismeldungDTO ergebnismeldung) {
    log.debug(
        "SENDERGEBNISSE BUSINESSAKTION #sendergebnis 3.1 a sendErgebnisseToEAI" + ergebnismeldung);
  ...
  private void assertWahlIsGeschlossen(final BezirkUndWahlID bezirkUndWahlID) {
    WahlbezirkArtModel wahlbezirkart =
        authenticationService.getWahlbezirkArtOfCurrentAuthenticationOrThrow();
    boolean isGeschlossen;
    try {
      isGeschlossen =
          switch (wahlbezirkart) {
            case UWB -> urnenwahlClient.isWahlbezirkGeschlossen(bezirkUndWahlID.getWahlbezirkID());
            case BWB -> {
              val umschlaegeOfWahlbezirk =
                  stimmzettelumschlaegeService.getStimmzettelumschlaege(bezirkUndWahlID);
              yield umschlaegeOfWahlbezirk
                  .filter(
                      stimmzettelumschlaegeModel ->
                          stimmzettelumschlaegeModel.urneneroeffnungsUhrzeit() != null)
                  .isPresent();
            }
          };
    } catch (final Exception e) {
      log.error(
          "#isWahlGeschlossen exception beim Versuch die Schließungsuhrzeit zu laden: {}",
          e.getMessage(),
          e);
      isGeschlossen = true;
    }
    ...
    if (!isGeschlossen) {
      log.error(
          "Es wurde keine Schließungsuhrzeit für die Wahl {} vom Bezirk {} erfasst",
  ...
  private void sendSendungsuhrzeiten(final ErgebnismeldungDTO ergebnismeldung) {
    ...
    try {
      val meldungsart =
          ergebnismeldung
              .getMeldungsart(); // meldungsart kann null sein, dann fliegt eine Exception!
      ...
      if (meldungsart.equals(ErgebnismeldungDTO.MeldungsartEnum.NIEDERSCHRIFT)) {
        statusClient.postNiederschriftSendungsuhrzeit(
            new BezirkUndWahlID(ergebnismeldung.getWahlID(), ergebnismeldung.getWahlbezirkID()),
            now);
      } else if (meldungsart.equals(ErgebnismeldungDTO.MeldungsartEnum.SCHNELLMELDUNG)) {
        statusClient.postSchnellmeldungSendungsuhrzeit(
            new BezirkUndWahlID(ergebnismeldung.getWahlID(), ergebnismeldung.getWahlbezirkID()),
            now);
      }
  ...
  private void logErgebnismeldungGesendet(final String meldungsart, final String wahlID) {
    val now = LocalDateTime.now();
    val eid =
        meldungsart.equals(MeldungsartModel.V3.name())
            ? "SCHNELLMELDUNG_GESENDET"
            : "NIEDERSCHRIFT_GESENDET";
