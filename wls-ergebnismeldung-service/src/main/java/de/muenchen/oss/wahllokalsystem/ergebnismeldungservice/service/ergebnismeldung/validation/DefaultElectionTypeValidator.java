...
public class DefaultElectionTypeValidator {
   ...
   * benötigten Daten aus den repos benötigt. Wird von den checkValidation(...) der abgeleiteten
   * Klassen gerufen, diese erzeugen auch benoetigteStapel.
   */
  public boolean checkValidation(
      ...
      List<Stapelart> benoetigteStapel)
      throws WlsException {
  ...
  private boolean checkValidationStapel(
      final String wahlbezirkID, final String wahlID, final List<Stapelart> benoetigteStapel)
      throws WlsException {

    // Ermittle vorhandene Stapelarten aus
    // ergebnisRepo.ergebnisseList.ergebnisse.bezirksUndWahlIdStapelart und
    ...
    for (Stapelart stapelart : benoetigteStapel) {
      if (!stapelartList.contains(stapelart)) {
        log.error(
            "Wahlbezirk {} hat Stapel {} noch nicht erfasst. Invalide!", wahlbezirkID, stapelart);
        val exceptionWrapperForStapel =
            new ExceptionDataWrapper(
                ExceptionConstants.SENDERGEBNISSE_STAPELN_UNVOLLSTAENDIG.code(),
                ExceptionConstants.SENDERGEBNISSE_STAPELN_UNVOLLSTAENDIG.message()
                    + " "
                    + stapelart);
  ...
  private boolean checkValidationStimmabgabevermerke(
    ...
    if (!stimmabgabevermerkeErfasst) {
      log.error(
          "Urnenwahlbezirk {} hat Stimmabgabevermerke noch nicht erfasst. Invalide!", wahlbezirkID);
  ...
  private boolean checkValidationWahlscheine(final String wahlbezirkID, final String wahlID)
      throws WlsException {
