...
public class NiederschriftSendungsuhrzeitSender extends AbstractStatusMonitoringSender {
  ...
  private boolean hasValidierungsstatusChanged(
    ...
    return oldStatus.niederschrift().validierungsstatus() != null
        && newStatus.niederschrift().validierungsstatus() != null
        && oldStatus.niederschrift().validierungsstatus()
            != newStatus.niederschrift().validierungsstatus();
  }
  ...
  private boolean hasValidierungsstatusExceptNichtValidiert(final StatusModel status) {
    return status.niederschrift() != null
        ...
        && !status
            .niederschrift()
            .validierungsstatus()
            .equals(ValidierungsstatusModel.NICHT_VALIDIERT);
  }
}
