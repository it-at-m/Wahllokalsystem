...
class BasisdatenClientImplTest {
  ...
  @Mock WahlenControllerApi wahlenControllerApi;
  @Mock ExceptionFactory exceptionFactory;
  @Mock BasisdatenClientMapper basisdatenClientMapper;

  @InjectMocks BasisdatenClientImpl unitUnderTest;
  ...
  class GetWahlart {
    ...
    void should_returnWahlartOfWahl_when_wahlWithIDExists() {
      val mockedWahlen =
          List.of(
              new WahlDTO().wahlart(WahlDTO.WahlartEnum.BTW).wahlID("other" + wahlID),
      ...
      Mockito.when(basisdatenClientMapper.toModel(WahlDTO.WahlartEnum.LTW))
          .thenReturn(WahlartModel.LTW);
    ...
    void should_throwFachlicheWlsException_when_noWahlWithGivenIDExistsOnCurrentWahltag() {
      ...
      val mockedWlsException =
          FachlicheWlsException.withCode("000").buildWithMessage("no wahl with wahlID given");
      ...
      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.getWahlart(wahltagID, wahlID))
    ...
    void should_throwFachlicheWlsException_when_noWahlenExistsOnCurrentWahltag() {
      ...
      val mockedWlsException =
          FachlicheWlsException.withCode("000").buildWithMessage("no wahlen on current wahltag");
