...
class InfomanagementClientImplTest {
  ...
  @InjectMocks InfomanagementClientImpl unitUnderTest;
  ...
  class GetWahltagID {
    ...
    void should_rethrowWlsException_when_apiThrewWlsException() {
      val mockedApiWlsException =
          TechnischeWlsException.withCode("000").buildWithMessage("wls exception from api");
      Mockito.doThrow(mockedApiWlsException)
          .when(konfigurierterWahltagControllerApi)
          .getKonfigurierterWahltag();

      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.getWahltagID())
    ...
    void should_throwTechnischeWlsException_when_apiThrewNonWlsException() {
      ...
      val mockedWlsException =
          TechnischeWlsException.withCode("000").buildWithMessage("transformed api exception");
    ...
    void should_throwFachlicheWlsException_when_apiReturnedNull() {
      val mockedWlsException =
          FachlicheWlsException.withCode("000").buildWithMessage("transformed api exception");
    ...
    void should_throwFachlicheWlsException_when_apiReturnedWahltagWithoutID() {
      ...
      Mockito.when(konfigurierterWahltagControllerApi.getKonfigurierterWahltag())
          .thenReturn(new KonfigurierterWahltagDTO().wahltagID(null));
      Mockito.when(
              exceptionFactory.createFachlicheWlsException(
                  ExceptionConstants.INFOMANAGEMENT_WAHLTAG_NULL_OR_EMPTY))
          .thenReturn(mockedWlsException);
