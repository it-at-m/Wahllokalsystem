...
class UnicodeConfigurationTest {
  ...
  @Autowired private BegruendungRepository begruendungRepository;
  ...
  void should_returnComposedString_when_givenDecomposedString() {
    ...
    val begruendungDTO = new BegruendungDTO(id, TEXT_ATTRIBUTE_DECOMPOSED, null, true, true);

    Assertions.assertThat(TEXT_ATTRIBUTE_DECOMPOSED.length())
        .isEqualTo(begruendungDTO.grund().length());

    // store Begruendung
    testRestTemplate.postForEntity(
        URI.create(
            ENTITY_ENDPOINT_URL + id.wahlbezirkID() + "/" + id.wahlID() + "/" + id.stapelart()),
        begruendungDTO,
        Void.class);
    ...
    val begruendung =
        begruendungRepository
            .findById(new BezirkUndWahlIDStapelart("bezirkID", "wahlID", Stapelart.LTW_BZW_A))
            .orElse(null);
    ...
    Assertions.assertThat(TEXT_ATTRIBUTE_COMPOSED.length())
        .isEqualTo(begruendung.getGrund1().length());
  }
}
