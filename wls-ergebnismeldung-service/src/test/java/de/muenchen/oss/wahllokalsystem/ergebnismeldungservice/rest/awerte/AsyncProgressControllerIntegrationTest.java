...
public class AsyncProgressControllerIntegrationTest {
  ...
  @MockitoSpyBean AsyncProgressDTOMapper asyncProgressDTOMapper;
  ...
  class GetAsyncProgress {
    ...
    void should_returnHttpStatusOKWithAsyncProgressDTO_when_serviceReturnsData() throws Exception {
      ...
      val response = api.perform(request).andExpect(status().isOk()).andReturn().getResponse();
      val asyncProgressDTO =
          objectMapper.readValue(response.getContentAsString(), AsyncProgressDTO.class);

      val expectedAsyncProgress = asyncProgressDTOMapper.toDTO(asyncProgress);

      Assertions.assertThat(asyncProgressDTO)
          .usingRecursiveComparison()
          .isEqualTo(expectedAsyncProgress);
    }
    ...
    void should_returnHttpStatus500WithWlsException_when_anExceptionOccurred() throws Exception {
      ...
      Mockito.when(asyncProgressDTOMapper.toDTO(any()))
      ...
      val response =
          api.perform(request)
              ...
              .andReturn()
              .getResponse();
      val responseAsWlsExceptionDTO =
          objectMapper.readValue(response.getContentAsString(), WlsExceptionDTO.class);

      val expectedWlsExceptionDTO =
          new WlsExceptionDTO(WlsExceptionCategory.T, "999", serviceIDFormatter.getId(), "");
