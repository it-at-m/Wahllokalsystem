...
class ErgebnismeldungControllerIntegrationTest {
  ...
  private static final MockedStatic<LocalDateTime> mockedStaticLocalDateTime =
  ...
  class SendErgebnisse {
    ...
    class UpdateSendungszeiten {
      ...
      class ForUWB {
        ...
        void
          ...
          val schnellmeldungSendungsuhrzeitStubbing =
              stubFor(
          ...
          val statusRequest =
              getAllServeEvents(
                      ServeEventQuery.forStubMapping(schnellmeldungSendungsuhrzeitStubbing))
                  .get(0);
          val sendStatusRequestBodyAsDTO =
              objectMapper.readValue(statusRequest.getRequest().getBody(), SendungsdatenDTO.class);

          val expectedSendStatusRequestBody =
              new SendungsdatenDTO()
                  .bezirkUndWahlID(
                      new de.muenchen.oss.wahllokalsystem.ergebnismeldungservice.eai.monitoring
                              .model.BezirkUndWahlID()
                          .wahlID(wahlID)
              ...
              MockMvcRequestBuilders.post(
                  ...
                  .with(
                      jwt()
                          ...
                          .jwt(
                              jwt ->
                                  jwt.claim("wahlbezirkID", "wahlbezirkID1_Wrong")
                                      .claim("wahlbezirksArt", "UWB")));
        ...
        void should_returnBadRequestWithFachlicheWlsException_when_wahlIsNotGeschlossen()
            throws Exception {
          val wahlbezirkID = "wahlbezirkID";
          val wahlID = "wahlID";
          val waehlerverzeichnisNummer = 1L;
          ...
          val resultBodyAsWlsExceptionDTO =
              objectMapper.readValue(
                  mockMvcResult.getResponse().getContentAsString(), WlsExceptionDTO.class);

          Assertions.assertThat(resultBodyAsWlsExceptionDTO.category())
              .isEqualTo(WlsExceptionCategory.F);
        }
      }
    }
    ...
    class SendErgebnisseToEAI {
            ...
            MockMvcRequestBuilders.post(
                ...
                .with(
                    jwt()
                        ...
                        .jwt(
                            jwt ->
                                jwt.claim("wahlbezirkID", "wahlbezirkID_wrong")
                                    .claim("wahlbezirksArt", "UWB")));
        mockMvc.perform(request).andExpect(status().isForbidden());
      }
      ...
      private void insertDataForErgebnismeldungIntoRepositories(
        ...
        val stapelA =
            new Ergebnisse(
                new BezirkUndWahlIDStapelart(wahlbezirkID, wahlID, Stapelart.EUW_A),
                List.of(new Ergebnis("wahlvorschlag1", null, 0L, 1L, 0L)));
        val stapelBUngekennzeichnet =
            new Ergebnisse(
                new BezirkUndWahlIDStapelart(
                    wahlbezirkID, wahlID, Stapelart.EUW_B_UNGEKENNZEICHNET),
                List.of(new Ergebnis("wahlvorschlag1", null, 0L, 1L, 0L)));
        val stapelCGueltig =
            new Ergebnisse(
                new BezirkUndWahlIDStapelart(wahlbezirkID, wahlID, Stapelart.EUW_C_GUELTIG),
                List.of(new Ergebnis("wahlvorschlag1", null, 0L, 1L, 0L)));
        val stapelCUngueltig =
            new Ergebnisse(
                new BezirkUndWahlIDStapelart(wahlbezirkID, wahlID, Stapelart.EUW_C_UNGUELTIG),
                List.of(new Ergebnis("wahlvorschlag1", null, 0L, 1L, 0L)));
  ...
  private String createURISendErgebnismeldungForNiederschrift(
      final String wahlbezirkID, final String wahlID, final long waehlerverzeichnisNummer) {
    return "/businessActions/sendErgebnismeldung/"
        + wahlID
        + "/"
        + wahlbezirkID
        + "/"
        + waehlerverzeichnisNummer
        + "/V1/hauptwahlbezirkID";
  }
