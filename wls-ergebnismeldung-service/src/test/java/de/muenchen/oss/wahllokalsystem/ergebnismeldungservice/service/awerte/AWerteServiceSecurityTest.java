...
class AWerteServiceSecurityTest {
  ...
  class GetAWerte {
    ...
    void should_grantAccessAndThrowNoException_when_allAuthoritiesUserGetAWerteIsValid()
      ...
      WireMock.stubFor(
          WireMock.get("/wahldaten/wahlbezirke/" + wahlbezirkID + "/wahlberechtigte")
              .willReturn(
                  WireMock.aResponse()
  ...
  class InitialiseAWerte {
    ...
    void should_failWithAccessDeniedException_when_adminLoadwahltermindatenAuthorityIsMissing() {
      ...
      Assertions.assertThatThrownBy(() -> aWerteService.initialiseAWerte(wahlbezirkIDList))
          .isInstanceOf(AccessDeniedException.class);
    }

    @Test
    void should_grantAccessAndThrowNoException_when_adminLoadwahltermindatenAuthorityIsValid()
        throws Exception {
      ...
      Assertions.assertThatNoException()
          .isThrownBy(() -> aWerteService.initialiseAWerte(wahlbezirkIDList));
      Assertions.assertThat(
              loggerExtension
                  .getLoggedEventsStream()
                  .filter(event -> event.getLevel() == Level.ERROR)
