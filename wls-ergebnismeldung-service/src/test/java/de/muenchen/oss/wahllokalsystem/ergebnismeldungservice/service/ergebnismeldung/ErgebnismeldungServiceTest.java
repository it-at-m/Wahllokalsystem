...
class ErgebnismeldungServiceTest {
  ...
  @Mock CurrentWahltagClient currentWahltagClient;
  @Mock EaiClient eaiClient;
  ...
  @InjectMocks ErgebnismeldungService unitUnderTest;
  ...
  class UpdateSendungszeiten {
    ...
    class ForUWB {
      ...
      void should_throwFachlicheWlsException_when_wahlIsNotGeschlossen() {
        ...
        val mockedFachlicheWlsException =
            FachlicheWlsException.withCode("000").buildWithMessage("wahl is nicht geschlossen");

        Mockito.when(
                exceptionFactory.createFachlicheWlsException(
                    ExceptionConstants.WAHL_NICHT_GESCHLOSSEN))
      ...
      void should_sendUpdateSendungszeiten_when_exceptionDuringIsWahlGeschlossenOccurred() {
        ...
        Mockito.when(authenticationService.getWahlbezirkArtOfCurrentAuthenticationOrThrow())
            .thenReturn(WahlbezirkArtModel.UWB);
        Mockito.when(statusService.getStatus(bezirkUndWahlID))
            .thenReturn(Optional.of(mockedStatus));
        Mockito.doThrow(mockedUrnenwahlClientException)
            .when(urnenwahlClient)
            .isWahlbezirkGeschlossen(wahlbezirkID);

        unitUnderTest.updateSendungszeiten(bezirkUndWahlID);

        Mockito.verify(statusClient)
            .postSchnellmeldungSendungsuhrzeit(eq(bezirkUndWahlID), eq(mockedNow));
      }
    }
    ...
    class ForBWB {
      ...
      void should_sendUpdateSendungszeiten_when_exceptionDuringIsWahlGeschlossenOccurred() {
        ...
        val mockedNow = LocalDateTime.now();
        val mockedStimmzettelumschlaegeService =
            new RuntimeException("exception on getStimmzettelumschlaege");
        val mockedStatus =
            new StatusModel(
                bezirkUndWahlID,
                createMeldungWithValidierungsstatus(ValidierungsstatusModel.VALIDE),
        ...
        Mockito.when(statusService.getStatus(bezirkUndWahlID))
            .thenReturn(Optional.of(mockedStatus));
        Mockito.doThrow(mockedStimmzettelumschlaegeService)
            .when(stimmzettelumschlaegeService)
            .getStimmzettelumschlaege(bezirkUndWahlID);
    ...
    @Test
    void
        should_sendSchnellmeldungSendungsuhrzeit_when_niederschriftWasNotSendButSchnellmeldungWasSent() {
      val wahlbezirkID = "wahlbezirkID";
      val wahlID = "wahlID";
      val bezirkUndWahlID = new BezirkUndWahlID(wahlID, wahlbezirkID);
    ...
    void should_sendNiederschriftSendungsuhrzeit_when_niederschriftWasNotNotSent() {
      ...
      Mockito.verify(statusClient)
          .postNiederschriftSendungsuhrzeit(eq(bezirkUndWahlID), eq(mockedNow));
    }
    ...
    void should_throwFachlicheWlsException_when_niederschriftAndSchnellmeldungWasNotSent() {
      ...
      Mockito.when(
              exceptionFactory.createFachlicheWlsException(
                  ExceptionConstants.FORCEERGEBNISSE_WRONG_USAGE))
          .thenReturn(mockedFachlicheWlsException);
    ...
    void should_callValidatorWithBezirkUndWahlID_when_called() {
      ...
      Mockito.doThrow(mockedValidationException)
          .when(ergebnismeldungValidator)
          .validBezirkUndWahlIDOrThrow(bezirkUndWahlID);
  ...
  class SendErgebnisse {
    ...
    class ForUWB {
      ...
      void should_throwFachlicheWlsException_when_wahlIsNotGeschlossen() {
        ...
        val ergebnisseToSendCriteria =
            new ErgebnisseToSendCriteriaModel(
                wahlID, wahlbezirkID, waehlerverzeichnisNummer, meldungsart, wahlbezirkID);
        ...
        Mockito.when(urnenwahlClient.isWahlbezirkGeschlossen(wahlbezirkID)).thenReturn(false);
        Assertions.assertThatException()
            .isThrownBy(() -> unitUnderTest.sendErgebnisse(ergebnisseToSendCriteria))
            .isSameAs(mockedFachlicheWlsException);
      }
    }
    ...
    class ForNiederschrift {
      ...
      void should_sendSendungsuhrzeiten_when_validErgebnisse() {
        ...
        val ergebnisseToSendCriteria =
            new ErgebnisseToSendCriteriaModel(
                wahlID, wahlbezirkID, waehlerverzeichnisNummer, meldungsart, hauptwahlbezirkID);
        ...
        val mockedErgebnismeldungDTO =
            new ErgebnismeldungDTO()
                .wahlID(wahlID)
                .wahlbezirkID(wahlbezirkID)
                .meldungsart(mockedEAIMeldungsart);
        val mockedNow = LocalDateTime.now();
        ...
        Mockito.when(
                ergebnismeldungValidator.checkValidation(
                    ...
                    eq(mockedWahlbezirkartOfCurrentUser),
                    eq(wahlbezirkID),
                    eq(wahlID),
                    eq(waehlerverzeichnisNummer),
                    eq(meldungsart)))
            .thenReturn(true);
        Mockito.when(currentWahltagClient.getWahltagID()).thenReturn(mockedCurrentActiveWahltagID);
        ...
        Mockito.verify(statusClient)
            .postNiederschriftSendungsuhrzeit(new BezirkUndWahlID(wahlID, wahlbezirkID), mockedNow);
      }
    }
      ...
      void should_sendSendungsuhrzeiten_when_validErgebnisse() {
        ...
        Mockito.verify(statusClient)
            .postSchnellmeldungSendungsuhrzeit(
                new BezirkUndWahlID(wahlID, wahlbezirkID), mockedNow);
      }
    }
    ...
    void should_callErgebnisValidator_when_called() {
      ...
      Mockito.verify(ergebnismeldungValidator)
          .validErgebnisseToSendCriteriaOrThrow(ergebnisseToSendCriteria);
      Mockito.verify(ergebnismeldungValidator)
          .checkValidation(
              eq(mockedWahlartOfCurrentWahltag),
              eq(mockedWahlbezirkartOfCurrentUser),
  ...
  private MeldungModel createMeldungWithValidierungsstatus(
      final ValidierungsstatusModel validierungsstatus) {
    return new MeldungModel(validierungsstatus, true, true, LocalDateTime.now());
  }

  private StimmzettelumschlaegeModel createStimmzettelumschlaegeWithUrnenOeffnungsUhrzeit(
      final LocalDateTime urnenOeffnungUhrszeit) {
