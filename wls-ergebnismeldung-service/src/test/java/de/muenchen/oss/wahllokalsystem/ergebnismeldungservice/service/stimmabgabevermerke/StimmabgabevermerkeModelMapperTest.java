...
class StimmabgabevermerkeModelMapperTest {
  ...
  final StimmabgabevermerkeModelMapper unitUnderTest =
      Mappers.getMapper(StimmabgabevermerkeModelMapper.class);
  ...
  class ToEntity {
    ...
    class OfStimmabgabevermerkeModel {
      ...
      void should_useStimmzettelartWithSameName_when_stimmzettelartModelIsGiven(
          final StimmzettelartModel stimmzettelartModel) {
        val vermerkToMap =
            new VermerkModel(0L, Set.of(new StimmzettelModel(0L, stimmzettelartModel)));

        val result = unitUnderTest.toEntity(vermerkToMap);

        Assertions.assertThat(result.getStimmzettel().iterator().next().getStimmzettelart().name())
            .isEqualTo(stimmzettelartModel.name());
      }
    }
  }
}
