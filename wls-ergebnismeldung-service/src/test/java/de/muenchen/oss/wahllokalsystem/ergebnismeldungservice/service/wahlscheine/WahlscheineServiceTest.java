...
class WahlscheineServiceTest {
  ...
  @Mock WahlscheineRepository wahlscheineRepository;

  @Mock WahlscheineModelMapper wahlscheineModelMapper;

  @Mock WahlscheineValidator wahlscheineValidator;

  @Mock ExceptionFactory exceptionFactory;

  @InjectMocks WahlscheineService unitUnderTest;
  ...
  class GetWahlscheine {
    ...
    void should_submitFachlicheWlsExceptionForParameter_when_callingValidator() {
      ...
      Mockito.when(
              exceptionFactory.createFachlicheWlsException(
                  ExceptionConstants.GET_WAHLSCHEINE_PARAMETER_UNVOLLSTAENDIG))
          .thenReturn(mockedWlsException);

      unitUnderTest.getWahlscheine(id);

      Mockito.verify(wahlscheineValidator)
    ...
    void should_returnWahlscheineModel_when_wahlscheineIsFoundFromRepo() {
      ...
      Mockito.when(wahlscheineModelMapper.toModel(mockedEntity))
  ...
  class SetWahlscheine {
    ...
    void should_saveMappedWahlscheineModel_when_called() {
      ...
      Mockito.when(wahlscheineModelMapper.toEntity(wahlscheineToSet))
    ...
    void should_throwTechnischeWlsException_when_savingFailed() {
      ...
      Mockito.doThrow(mockedRepositorySaveException)
          .when(wahlscheineRepository)
          .save(mockedModelAsEntity);
      Mockito.when(
              exceptionFactory.createTechnischeWlsException(
                  ExceptionConstants.WAHLSCHEINE_UNSAVEABLE))
          .thenReturn(mockedExceptionFactoryWlsException);

      Assertions.assertThatThrownBy(() -> unitUnderTest.setWahlscheine(id, wahlscheineToSet))
