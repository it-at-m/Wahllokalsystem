...
console.info(
    logID + '1 nicer Wahl Arbeiter am Start vong her.'
);
...
self.handlePOST = (event) => {
    ...
    let key = event.request.url,
        ...
        contentType = clonedRequest.headers.has(ContentTypes.HEADERNAME) ? clonedRequest.headers.get(ContentTypes.HEADERNAME) : ContentTypes.JSON;
    return self.blobOrTxtProm(clonedRequest, contentType).then((data) => {
        // ergebnismeldung senden soll status immer weiterreichen
        let is_ergebnismeldung_send = event.request.url.includes('ergebnismeldung/businessActions/sendErgebnismeldung');
        return self.fetchEvent(event)
            .then(function (response) {

                // als dirty speichern wenn wir nicht ok zurückbekommen
                let dirty = StatusCodeOk.indexOf(response.status) === -1;
                return setItemAndContentType(key, data, contentType, dirty).then(function () {
                    // redirect, 200er, und ergebnnismeldung senden ergebnise durchreichen
                    if (response.type === "opaqueredirect" || dirty === false || is_ergebnismeldung_send)
                        return response;
...
self.handleGET = (event) => {
    ...
    return getItem(key)
        .then(_handleGetIDB)
        .catch(() => {
            return self.fetchEvent(event).then((response) => self._handleGetFetch(key, response, false))
        })
}
...
self.handleGETonlineFirst = (event) => {
    ...
    let key = event.request.url;
    return self.fetchEvent(event).then((response) => self._handleGetFetch(key, response, true))
        .catch((badResponse) => {
            return getItem(key)
                .then((data) => self._handleGetIDB(data, badResponse))
        })
}
...
self._handleGetFetch = (key, response, rejectResponse = false) => {
    ...
    let responseClone = response.clone(),
        contentType = response.headers.has(ContentTypes.HEADERNAME) ? response.headers.get(ContentTypes.HEADERNAME) : ContentTypes.JSON;
    if (StatusCodeOk.indexOf(response.status) !== -1) {
        return self.blobOrTxtProm(response, contentType).then((data) => {

            return setItemAndContentType(key, data, contentType, false, response.status).then(() => {
...
self._handleGetIDB = (data, responseIfNoData) => {
    ...
    if (data === null) {
        ...
        return responseIfNoData ? Promise.reject(responseIfNoData) : Promise.reject();
    } else {
        var init = {
            status: data.status,
...
self.blobOrTxtProm = (request, contentType) => {
    ...
    if (contentType && (contentType.indexOf(ContentTypes.PDF) !== -1 || contentType.indexOf(ContentTypes.CSV) !== -1)) {
        return request.blob()
    } else {
        ...
        getPin().then(() => {
            localforage.iterate(function (value, key, iterationNumber) {
                if (key !== PIN_KEY) {
                    ...
                    if (value.contentType.includes(ContentTypes.JSON) && value.data && value.data !== "") {
                        try {
...
function setItemAndContentType(key, value, contentType, dirty, responseStatus) {
    ...
    if (responseStatus === 204) {
        ...
        let encrypted = value instanceof Blob ? value : CryptoJS.AES.encrypt(value, self.pin).toString();
        contentTypeData = {
            data: encrypted,
            contentType: contentType,
            dirty: dirty,
...
function getItem(key) {
    return localforage.getItem(key).then((item) => {
        ...
        if (!(item.data instanceof Blob)) {
            if (item.status !== 204) {
                decrypted = CryptoJS.AES.decrypt(item.data, self.pin);
...
function getPin() {
    return localforage.getItem(PIN_KEY).then((pin) => {
        self.pin = pin;
        self.state = self.pin === undefined || self.pin === null ? WahlworkerState.INACTIVE : WahlworkerState.ACTIVE; // kann die idb nicht verwenden wenn wir keine pin haben.
    });
}
