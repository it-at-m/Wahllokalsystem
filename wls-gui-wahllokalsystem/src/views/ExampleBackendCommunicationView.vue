...
const error = ref("");
const wahlbezirkID1 = "wbz-1";

function getApi() {
  fetch("/api/backend-service/businessActions/getMessage/" + wahlbezirkID1)
    .then((response) => {
      if (response.status == 204) {
        console.log(response);
        throw new Error("Es konnte keine Nachricht gefunden werden. (204)");
      }
      if (!response.ok) {
    ...
    .catch((err) => {
      error.value = "Es gibt keine neuen Nachrichten";
