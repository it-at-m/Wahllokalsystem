...
describe("stimmzettelErfassungViewButtonStateUtils.ts", () => {
  ...
  describe("startenBtnIsDisabled", () => {
    const startenBtnIsDisabledCases: TestCases = [
      [
        "ABGESCHLOSSEN",
        { status: StimmzettelerfassungTeamStatusEnum.ABGESCHLOSSEN },
        true,
    ...
    it.each(startenBtnIsDisabledCases)(
      "should_haveCorrectState_when_statusIs%s",
      (_label, teamStatus, expected) => {
        const teamStatusRef = ref<StimmzettelerfassungTeamStatus | null>(
          teamStatus
        ...
        expect(unitUnderTest.startenBtnIsDisabled.value).toBe(expected);
      }
    );

    it("should_coverAllEnumValues_whenTestCasesAreDefined", () => {
      verifyThatTestCasesCoverAllEnumValues(startenBtnIsDisabledCases);
    });
  });

  describe("beendenBtnIsDisabled", () => {
    const beendenBtnIsDisabledCases: TestCases = [
      [
        "ABGESCHLOSSEN",
        { status: StimmzettelerfassungTeamStatusEnum.ABGESCHLOSSEN },
        true,
    ...
    it.each(beendenBtnIsDisabledCases)(
      "should_haveCorrectState_when_statusIs%s",
      (_label, teamStatus, expected) => {
        const teamStatusRef = ref<StimmzettelerfassungTeamStatus | null>(
          teamStatus
        ...
        expect(unitUnderTest.beendenBtnIsDisabled.value).toBe(expected);
      }
    );

    it("should_coverAllEnumValues_whenTestCasesAreDefined", () => {
      verifyThatTestCasesCoverAllEnumValues(beendenBtnIsDisabledCases);
    });
  });
