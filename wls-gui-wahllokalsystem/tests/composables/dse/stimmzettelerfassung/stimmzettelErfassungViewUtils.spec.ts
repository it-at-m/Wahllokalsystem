...
describe("stimmzettelErfassungViewUtils.ts", () => {
  ...
  describe("setActiveStimmzettel", () => {
    it("should_setActiveStimmzettel_when_stimmzettelkennungExistsInSaved", async () => {
      ...
      await unitUnderTest.saveOrUpdateStimmzettel(stimmzettel1);
      await unitUnderTest.saveOrUpdateStimmzettel(stimmzettel2);

      unitUnderTest.setActiveStimmzettel(2);

      expect(unitUnderTest.activeStimmzettel.value).toStrictEqual(stimmzettel2);
    });

    it("should_keepActiveStimmzettelUnchanged_when_stimmzettelkennungNotInSaved", async () => {
      const stimmzettel = preparePersistedStimmzettel()
        .stimmzettelkennung(1)
        .build();
      mockDefinitions.saveStimmzettel.mockResolvedValue(undefined);

      await unitUnderTest.saveOrUpdateStimmzettel(stimmzettel);

      unitUnderTest.setActiveStimmzettel(stimmzettel.stimmzettelkennung);
      expect(unitUnderTest.activeStimmzettel.value).toStrictEqual(stimmzettel);
      unitUnderTest.setActiveStimmzettel(99);
      expect(unitUnderTest.activeStimmzettel.value).toStrictEqual(stimmzettel);
    });

    it("should_switchActiveStimmzettel_when_calledMultipleTimes", async () => {
      const stimmzettel1 = preparePersistedStimmzettel()
        .stimmzettelkennung(1)
        .build();
      const stimmzettel2 = preparePersistedStimmzettel()
      ...
      await unitUnderTest.saveOrUpdateStimmzettel(stimmzettel1);
      await unitUnderTest.saveOrUpdateStimmzettel(stimmzettel2);
      await unitUnderTest.saveOrUpdateStimmzettel(stimmzettel3);

      unitUnderTest.setActiveStimmzettel(1);
      expect(unitUnderTest.activeStimmzettel.value).toStrictEqual(stimmzettel1);

      unitUnderTest.setActiveStimmzettel(3);
      expect(unitUnderTest.activeStimmzettel.value).toStrictEqual(stimmzettel3);
    });
  });
});
