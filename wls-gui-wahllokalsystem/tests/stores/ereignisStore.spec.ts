  ...
  describe("areKeineEreignisseFlagsValid", () => {
    describe("should_returnExpectedValue_dependingOnFunctionData", () => {
      it.each(generateTestdataForAreKeineEreignisseFlagsValid())(
        "hasVorfaelle=$data.vorfaelle | hasVorfaelle=$data.vorkommnisse | wahlbezirkKeineVorfaelle=$data.wahlbezirkKeineVorfaelle | wahlbezirkKeineVorkomnisse=$data.wahlbezirkKeineVorkomnisse | wahlbezirkArt=$data.wahlbezirkArt | schliessungsuhrzeit=$data.schliessungsUhrzeitSent --> Expected=$expected",
        ({ data, expected }) => {
