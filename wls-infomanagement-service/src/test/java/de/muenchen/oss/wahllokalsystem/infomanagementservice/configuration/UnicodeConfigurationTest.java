...
class UnicodeConfigurationTest {
  ...
  private static final String ENTITY_ENDPOINT_URL =
      "/businessActions/konfiguration/WILLKOMMENSTEXT";
  ...
  @Autowired private KonfigurationRepository konfigurationRepository;

  @Test
  void should_returnComposedString_when_givenDecomposedString() {
    val konfigurationDTO =
        KonfigurationDTO.builder()
            .schluessel("WILLKOMMENSTEXT")
            .beschreibung(TEXT_ATTRIBUTE_DECOMPOSED)
            .build();

    Assertions.assertThat(konfigurationDTO.beschreibung())
        .hasSize(TEXT_ATTRIBUTE_DECOMPOSED.length());
    ...
    Assertions.assertThat(konfiguration.getBeschreibung())
        .hasSize(TEXT_ATTRIBUTE_COMPOSED.length());
  }
}
