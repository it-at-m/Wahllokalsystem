...
public class KonfigurationControllerIntegrationTest {
  ...
  @MockitoSpyBean KonfigurationModelValidator konfigurationModelValidator;
  ...
  class GetKonfiguration {
    ...
    @WithMockUserAsJwt(
        authorities = {
          Authorities.SERVICE_GET_KONFIGURATION,
          Authorities.REPOSITORY_READ_KONFIGURATION
        })
    ...
    void should_returnData_when_dataIsPresentInRepository() throws Exception {
      val konfiguration1 =
          new Konfiguration("schluessel1", "wert1", "beschreibung1", "standardwert1");
      val konfiguration2 =
          new Konfiguration("schluessel2", "wert2", "beschreibung2", "standardwert2");
      val konfigurationToFind =
          new Konfiguration(
              "WILLKOMMENSTEXT",
              "hello world",
      ...
      val responseBody =
          objectMapper.readValue(
              response.getResponse().getContentAsString(), KonfigurationDTO.class);

      val expectedResponseBody =
          new KonfigurationDTO(
              konfigurationToFind.getSchluessel(),
              konfigurationToFind.getWert(),
              konfigurationToFind.getBeschreibung(),
              konfigurationToFind.getStandardwert());
  ...
  class PostKonfiguration {
    ...
    void should_persistData_when_noDataIsPresentInRepository() throws Exception {
      ...
      val expectedSavedKonfiguration =
          new Konfiguration(
              konfigurationKey,
              requestBody.wert(),
              requestBody.beschreibung(),
              requestBody.standardwert());
    ...
    void should_replaceOldData_when_dataIsPresentInRepository() throws Exception {
      ...
      val konfigurationToOverride =
          new Konfiguration(konfigurationKey, "old value", "old description", "old default");
    ...
    void should_returnInternalServerErrorWLSException_when_validationFailed() throws Exception {
      ...
      Mockito.doThrow(mockedValidationException)
          ...
          .validOrThrowSetKonfiguration(any());
      ...
      val expectedWlsExceptionDTO =
          new WlsExceptionDTO(WlsExceptionCategory.T, "999", "WLS-INFOMANAGEMENT", "");
    ...
    private MockHttpServletRequestBuilder createPostWithBody(
        final String konfigurationKey, final KonfigurationSetDTO requestDTO) throws Exception {
      return MockMvcRequestBuilders.post("/businessActions/konfiguration/" + konfigurationKey)
          .with(csrf())
  ...
  class GetKonfigurationen {
    ...
    @WithMockUserAsJwt(
        authorities = {
          Authorities.SERVICE_GET_KONFIUGRATIONEN,
          Authorities.REPOSITORY_READ_KONFIGURATION,
          Authorities.REPOSITORY_WRITE_KONFIGURATION
        })
    void should_returnData_when_dataIsPresentInRepository() throws Exception {
      konfigurationRepository.save(
          new Konfiguration("schluessel1", "wert1", "beschreibung1", "standard1"));
      konfigurationRepository.save(
          new Konfiguration("schluessel2", "wert2", "beschreibung2", "standard2"));
  ...
  class GetKennbuchstabenListen {
    ...
    @WithMockUserAsJwt(
        authorities = {
          Authorities.SERVICE_GET_KENNBUCHSTABEN_LISTEN,
          Authorities.REPOSITORY_READ_KONFIGURATION,
          Authorities.REPOSITORY_WRITE_KONFIGURATION
        })
    void should_returnData_when_dataIsPresentInRepository() throws Exception {
      konfigurationRepository.save(
          new Konfiguration("KENNBUCHSTABEN", "a,b, c;A,B,C$1,2;11,12", "", ""));
      ...
      val responseBodyDTO =
          objectMapper.readValue(
              response.getResponse().getContentAsString(), KennbuchstabenListenDTO.class);

      val expectedResponseDTO =
          new KennbuchstabenListenDTO(
              List.of(
                  new KennbuchstabenListeDTO(
                      List.of(
                          new KennbuchstabenDTO(List.of("a", "b", " c")),
                          new KennbuchstabenDTO(List.of("A", "B", "C")))),
                  new KennbuchstabenListeDTO(
                      List.of(
                          new KennbuchstabenDTO(List.of("1", "2")),
                          new KennbuchstabenDTO(List.of("11", "12"))))));
  ...
  class GetKonfigurationUnauthorized {
    ...
    void should_returnData_when_dataIsPresentInRepository() throws Exception {
      ...
      val request =
          MockMvcRequestBuilders.get("/businessActions/konfigurationUnauthorized/WILLKOMMENSTEXT");

      val result = api.perform(request).andExpect(status().isOk()).andReturn();
      val resultBodyDTO =
          objectMapper.readValue(result.getResponse().getContentAsString(), KonfigurationDTO.class);
