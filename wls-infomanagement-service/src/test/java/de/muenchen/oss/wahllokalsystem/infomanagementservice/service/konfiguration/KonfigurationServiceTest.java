...
class KonfigurationServiceTest {
  @Mock KonfigurationRepository konfigurationRepository;

  @Mock KonfigurationModelMapper konfigurationModelMapper;

  @Mock KonfigurationModelValidator konfigurationModelValidator;

  @Mock ExceptionFactory exceptionFactory;

  @Mock JWTDetailRetriever jwtDetailRetriever;

  @Spy ArrayList<AuthDetailRetriever> authDetailRetrievers;

  @InjectMocks KonfigurationService unitUnderTest;
  ...
  class GetKonfiguration {
    ...
    void should_returnKonfigurationModel_when_foundInRepoWithAlternativKey() {
      ...
      Mockito.doNothing()
          .when(konfigurationModelValidator)
          .validOrThrowGetKonfigurationByKey(alternativeKey);
      Mockito.when(jwtDetailRetriever.canHandle(any())).thenReturn(true);
      Mockito.when(jwtDetailRetriever.getDetail(eq(JWT_DETAIL_WAHLBEZIRKSART_KEY), any()))
          .thenReturn(Optional.of("BWB"));
      Mockito.when(
              konfigurationModelMapper.getAlternativeKey(
                  keyForRequestedKonfiguration, WahlbezirkArt.BWB))
          .thenReturn(Optional.of(alternativeKey));
      Mockito.when(konfigurationModelMapper.toModel(mockedKonfigurationFromRepo))
          .thenReturn(mappedMockedKonfiguration);
      Mockito.when(konfigurationRepository.findById(alternativeKey.name()))
          .thenReturn(mockedKonfigurationFromRepoAsOptional);
    ...
    void should_returnKonfigurationModel_when_foundInRepoWithoutAlternativeKey() {
      ...
      Mockito.doNothing()
          .when(konfigurationModelValidator)
          .validOrThrowGetKonfigurationByKey(keyForRequestedKonfiguration);
      Mockito.when(jwtDetailRetriever.canHandle(any())).thenReturn(true);
      Mockito.when(jwtDetailRetriever.getDetail(eq(JWT_DETAIL_WAHLBEZIRKSART_KEY), any()))
          .thenReturn(Optional.of("BWB"));
      Mockito.when(
      ...
      Mockito.when(konfigurationModelMapper.toModel(mockedKonfigurationFromRepo))
          .thenReturn(mappedMockedKonfiguration);
      Mockito.when(konfigurationRepository.findById(konfigKeyAsString))
          .thenReturn(mockedKonfigurationFromRepoAsOptional);
    ...
    void should_throwException_when_validationFails() {
      ...
      Assertions.assertThatThrownBy(
              () -> unitUnderTest.getKonfiguration(keyForRequestedKonfiguration))
    ...
    void should_returnKonfigurationModel_when_wahlbezirksArtISNotPartOfJWT() {
      ...
      Mockito.when(
              konfigurationModelMapper.getAlternativeKey(
                  keyForRequestedKonfiguration, WahlbezirkArt.UWB))
          .thenReturn(Optional.empty());
      Mockito.when(konfigurationModelMapper.toModel(mockedKonfigurationFromRepo))
          .thenReturn(mappedMockedKonfiguration);
      Mockito.when(konfigurationRepository.findById(konfigKeyAsString))
    ...
    void should_returnKonfigurationModel_when_noAuthenticationHandlerIsFound() {
      ...
      SecurityContextHolder.getContext()
          .setAuthentication(new UsernamePasswordAuthenticationToken("princiap", "credential"));
  ...
  class SetKonfiguration {
    ...
    void should_saveKonfiguration_when_calledWithModel() {
      ...
      Mockito.doNothing()
          .when(konfigurationModelValidator)
          .validOrThrowSetKonfiguration(konfigurationSetModel);
      Mockito.when(konfigurationModelMapper.toEntity(konfigurationSetModel))
          .thenReturn(mockedKonfigurationEntity);

      Assertions.assertThatNoException()
          .isThrownBy(() -> unitUnderTest.setKonfiguration(konfigurationSetModel));
    ...
    void should_throwException_when_validationFails() {
      ...
      Assertions.assertThatThrownBy(() -> unitUnderTest.setKonfiguration(konfigurationSetModel))
    ...
    void should_throwTechnischeWlsException_when_repoFails() {
      ...
      val mockedExceptionFactoryWlsException =
          TechnischeWlsException.withCode("").buildWithMessage("");
      val mockedKonfigurationEntity = new Konfiguration();
      ...
      Mockito.when(konfigurationModelMapper.toEntity(konfigurationSetModel))
          .thenReturn(mockedKonfigurationEntity);
      Mockito.when(konfigurationRepository.save(mockedKonfigurationEntity))
  ...
  class GetKennbuchstabenListen {
    ...
    void should_returnKennbuchstabenListenModel_when_dataFoundInRepo() {
      ...
      Mockito.when(konfigurationRepository.findById("KENNBUCHSTABEN"))
          .thenReturn(Optional.of(mockedRepoResponse));
      Mockito.when(konfigurationModelMapper.toKennbuchstabenListenModel(konfigurationWert))
    ...
    void should_returnFachlicheWlsException_when_noDataFoundInRepo() {
      ...
      Assertions.assertThatException()
          .isThrownBy(() -> unitUnderTest.getKennbuchstabenListen())
          .isSameAs(mockedExceptionFactoryWlsException);
    }
  }
  ...
  class GetKonfigurationUnauthorized {
    ...
    private static final List<KonfigurationKonfigKey> SUPPORTED_KEYS =
        List.of(
            KonfigurationKonfigKey.FRUEHESTE_LOGIN_UHRZEIT,
            KonfigurationKonfigKey.SPAETESTE_LOGIN_UHRZEIT,
            KonfigurationKonfigKey.WILLKOMMENSTEXT);

    @ParameterizedTest
    @MethodSource("getNonSupportedKeys")
    void should_verifyThatRepoIsNotCalled_when_nonSupportedKeysAreGiven(
    ...
    void should_returnKonfigurationModel_when_fruehesteLoginUhrzeitIsGiven() {
      ...
      Mockito.when(konfigurationRepository.getFruehesteLoginUhrzeit())
          .thenReturn(Optional.of(mockedRepoResponse));
      Mockito.when(konfigurationModelMapper.toModel(mockedRepoResponse))
          .thenReturn(mockedMappedEntityAsModel);

      val result =
          unitUnderTest.getKonfigurationUnauthorized(
              KonfigurationKonfigKey.FRUEHESTE_LOGIN_UHRZEIT);
    ...
    void should_returnKonfigurationModel_when_spaetesteLoginUhrzeitIsGiven() {
      ...
      Mockito.when(konfigurationRepository.getSpaetesteLoginUhrzeit())
          .thenReturn(Optional.of(mockedRepoResponse));
      Mockito.when(konfigurationModelMapper.toModel(mockedRepoResponse))
          .thenReturn(mockedMappedEntityAsModel);
    ...
    void should_returnKonfigurationModel_when_willkommenstextIsGiven() {
      ...
      Mockito.when(konfigurationRepository.getWillkommenstext())
          .thenReturn(Optional.of(mockedRepoResponse));
      Mockito.when(konfigurationModelMapper.toModel(mockedRepoResponse))
          .thenReturn(mockedMappedEntityAsModel);

      val result =
          unitUnderTest.getKonfigurationUnauthorized(KonfigurationKonfigKey.WILLKOMMENSTEXT);

      Assertions.assertThat(result.get()).isSameAs(mockedMappedEntityAsModel);
    }

    private static Stream<Arguments> getNonSupportedKeys() {
      return Arrays.stream(KonfigurationKonfigKey.values())
          .filter(konfigKey -> !SUPPORTED_KEYS.contains(konfigKey))
          .map(Arguments::of);
    }
  }
}
