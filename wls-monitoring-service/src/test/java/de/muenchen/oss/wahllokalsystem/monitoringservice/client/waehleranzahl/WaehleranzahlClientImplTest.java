...
class WaehleranzahlClientImplTest {
  ...
  @Mock WahlbeteiligungControllerApi wahlbeteiligungControllerApi;

  @Mock WaehleranzahlClientMapper waehleranzahlClientMapper;

  @InjectMocks WaehleranzahlClientImpl unitUnderTest;
  ...
  class PostWahlbeteiligung {
    ...
    void should_callEaiApiWithDTO_when_clientIsCalledWithModel() {
      val waehleranzahlModel =
          new WaehleranzahlModel(
              new BezirkUndWahlID("wahlID01", "wahlbezirkID01"), 99L, LocalDateTime.now());
      val mockedWahlbeteiligungsMeldungDTO = createWahlbeteiligungsMeldungDTO();
      Mockito.when(waehleranzahlClientMapper.fromModelToRemoteClientDTO(waehleranzahlModel))
          .thenReturn(mockedWahlbeteiligungsMeldungDTO);

      unitUnderTest.postWahlbeteiligung(waehleranzahlModel);
      Mockito.verify(wahlbeteiligungControllerApi)
          .saveWahlbeteiligung(mockedWahlbeteiligungsMeldungDTO);
    }
    ...
    void should_notThrowException_when_eaiApiThrowsAnyException() {
      ...
      Mockito.doThrow(mockedApiException)
          .when(wahlbeteiligungControllerApi)
          .saveWahlbeteiligung(mockedWahlbeteiligungsMeldungDTO);

      Assertions.assertThatNoException()
          .isThrownBy(() -> unitUnderTest.postWahlbeteiligung(waehleranzahlModel));
    }
    ...
    private WahlbeteiligungsMeldungDTO createWahlbeteiligungsMeldungDTO() {
      ...
      return new WahlbeteiligungsMeldungDTO()
          .wahlID(wahlID)
