...
public class WaehleranzahlControllerTest {
  ...
  @Mock WaehleranzahlService waehleranzahlService;

  @Mock WaehleranzahlDTOMapper waehleranzahlDTOMapper;

  @InjectMocks WaehleranzahlController unitUnderTest;
  ...
  class GetWahlbeteiligung {
    ...
    void should_returnHttp200AndData_when_dataIsFound() {
      ...
      Mockito.when(waehleranzahlService.getWahlbeteiligung(bezirkUndWahlID))
          .thenReturn(Optional.of(mockedServiceModel));
      Mockito.when(waehleranzahlDTOMapper.toDTO(mockedServiceModel))
  ...
  class PostWahlbeteiligung {
    ...
    void should_notThrowException_when_serviceIsCalled() {
      ...
      Mockito.when(waehleranzahlDTOMapper.toSetModel(bezirkUndWahlID, requestBody))
