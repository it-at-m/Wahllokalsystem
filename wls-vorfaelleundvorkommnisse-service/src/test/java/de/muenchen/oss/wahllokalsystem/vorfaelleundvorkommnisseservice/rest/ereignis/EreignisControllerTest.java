...
public class EreignisControllerTest {
  ...
  @Mock EreignisService ereignisService;

  @Mock EreignisDTOMapper ereignisDTOMapper;

  @InjectMocks EreignisController unitUnderTest;
  ...
  class GetEreignisse {
    ...
    void should_returnWahlbezirkEreignisseDTO_when_givenValidWahlbezirkid() {
      ...
      val expectedWahlbezirkEreignisseDto =
          TestdataFactory.CreateWahlbezirkEreignisseDto.fromModel(mockedEreignisseModel);

      Mockito.when(ereignisService.getEreignisse(wahlbezirkID))
          .thenReturn(Optional.of(mockedEreignisseModel));
      Mockito.when(ereignisDTOMapper.toDTO(mockedEreignisseModel))
          .thenReturn(expectedWahlbezirkEreignisseDto);
  ...
  class PostEreignisse {
    ...
    void should_notThrowException_when_newDataSaved() {
      ...
      val mockedEreignisseWriteModel =
          TestdataFactory.CreateEreignisseWriteModel.fromDto(
              wahlbezirkID, mockedEreignisseWriteDto);

      Mockito.when(ereignisDTOMapper.toModel(wahlbezirkID, mockedEreignisseWriteDto))
          .thenReturn(mockedEreignisseWriteModel);

      Assertions.assertThatNoException()
          .isThrownBy(() -> unitUnderTest.postEreignisse(wahlbezirkID, mockedEreignisseWriteDto));
