...
public class EroeffnungsUhrzeitControllerTest {
  ...
  @Mock EroeffnungsUhrzeitService eroeffnungsUhrzeitService;

  @Mock EroeffnungsUhrzeitDTOMapper eroeffnungsUhrzeitDTOMapper;

  @InjectMocks EroeffnungsUhrzeitController unitUnderTest;
  ...
  class GetEroeffnungsuhrzeit {
    ...
    void should_returnEroeffnungsuhrzeit_when_dataFound() {
      ...
      val mockedMappedServiceResponseAsDTO =
          new EroeffnungsUhrzeitDTO(wahlbezirkID, eroeffnungsUhrzeit);

      Mockito.when(eroeffnungsUhrzeitService.getEroeffnungsUhrzeit(wahlbezirkID))
          .thenReturn(Optional.of(mockedServiceOptionalBody));
      Mockito.when(eroeffnungsUhrzeitDTOMapper.toDTO(mockedServiceOptionalBody))
  ...
  class PostEroeffnungsuhrzeit {
    ...
    void should_postEroeffnungsuhrzeit_when_calledAndMappedCorrectly() {
      ...
      Mockito.when(eroeffnungsUhrzeitDTOMapper.toModel(eq(wahlbezirkID), eq(requestBody)))
          .thenReturn(mockedMappedRequest);

      Assertions.assertThatNoException()
          .isThrownBy(() -> unitUnderTest.postEroeffnungsuhrzeit(wahlbezirkID, requestBody));
