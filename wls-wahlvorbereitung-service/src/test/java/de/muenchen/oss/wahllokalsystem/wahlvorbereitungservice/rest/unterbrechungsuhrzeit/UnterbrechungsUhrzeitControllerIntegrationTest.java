...
public class UnterbrechungsUhrzeitControllerIntegrationTest {
  ...
  @Autowired UnterbrechungsUhrzeitModelMapper unterbrechungsUhrzeitModelMapper;

  @Autowired UnterbrechungsUhrzeitDTOMapper unterbrechungsUhrzeitDTOMapper;

  @Autowired UnterbrechungsUhrzeitRepository unterbrechungsUhrzeitRepository;
  ...
  class GetUnterbrechungsUhrzeit {
    ...
    @WithMockUser(
        authorities = {
          Authorities.SERVICE_UNTERBRECHUNGSUHRZEIT,
    ...
    void should_returnData_when_dataIsPresentInRepo() throws Exception {
      ...
      val responseBodyAsDTO =
          objectMapper.readValue(
              response.getResponse().getContentAsString(), UnterbrechungsUhrzeitDTO.class);
  ...
  class PostUnterbrechungsUhrzeit {
    ...
    void should_setNewData_when_callingPost() throws Exception {
      ...
      val unterbrechungsUhrzeitFromRepo =
          unterbrechungsUhrzeitRepository.findById(wahlbezirkID).get();
      val expectedUnterbrechungsUhrzeit =
          unterbrechungsUhrzeitModelMapper.toEntity(
              unterbrechungsUhrzeitDTOMapper.toModel(wahlbezirkID, writeDto));
    ...
    void should_replaceData_when_dataIsPresent() throws Exception {
      ...
      SecurityUtils.runWith(Authorities.REPOSITORY_READ_UNTERBRECHUNGSUHRZEIT);
      val unterbrechungsUhrzeitFromRepo1 =
          unterbrechungsUhrzeitRepository.findById(wahlbezirkID).get();
      val expectedUnterbrechungsUhrzeit1 =
          unterbrechungsUhrzeitModelMapper.toEntity(
              unterbrechungsUhrzeitDTOMapper.toModel(wahlbezirkID, writeDto1));

      Assertions.assertThat(unterbrechungsUhrzeitFromRepo1)
          .isEqualTo(expectedUnterbrechungsUhrzeit1);
      ...
      SecurityUtils.runWith(Authorities.REPOSITORY_READ_UNTERBRECHUNGSUHRZEIT);
      val unterbrechungsUhrzeitFromRepo2 =
          unterbrechungsUhrzeitRepository.findById(wahlbezirkID).get();
      val expectedUnterbrechungsUhrzeit2 =
          unterbrechungsUhrzeitModelMapper.toEntity(
              unterbrechungsUhrzeitDTOMapper.toModel(wahlbezirkID, writeDto2));

      Assertions.assertThat(unterbrechungsUhrzeitFromRepo2)
          .isEqualTo(expectedUnterbrechungsUhrzeit2);
    }
