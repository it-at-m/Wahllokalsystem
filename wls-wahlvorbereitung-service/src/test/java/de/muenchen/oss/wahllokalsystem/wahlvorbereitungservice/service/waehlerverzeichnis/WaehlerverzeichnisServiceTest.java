...
class WaehlerverzeichnisServiceTest {
  ...
  @Mock WaehlerverzeichnisRepository waehlerverzeichnisRepository;

  @Mock WaehlerverzeichnisValidator waehlerverzeichnisValidator;

  @Mock WaehlerverzeichnisModelMapper waehlerverzeichnisModelMapper;

  @Mock ExceptionFactory exceptionFactory;

  @InjectMocks WaehlerverzeichnisService unitUnderTest;
  ...
  class GetWaehlerverzeichnis {
    ...
    void should_returnWaehlerverzeichnis_when_givenValidReference() {
      val waehlerverzeichnisReference =
          new BezirkIDUndWaehlerverzeichnisNummer("wahlbezirkID", 123L);

      val mockedRepoResponse = new Waehlerverzeichnis();
      val mockedResponseAsModel = WaehlerverzeichnisModel.builder().build();

      Mockito.when(waehlerverzeichnisRepository.findById(waehlerverzeichnisReference))
          .thenReturn(Optional.of(mockedRepoResponse));
      Mockito.when(waehlerverzeichnisModelMapper.toModel(mockedRepoResponse))
          .thenReturn(mockedResponseAsModel);
    ...
    void should_throwException_when_validationFailed() {
      ...
      Mockito.doThrow(mockedValidationException)
          ...
          .validWaehlerverzeichnisReferenceOrThrow(waehlerverzeichnisReference);
  ...
  class SetWaehlerverzeichnis {
    ...
    void should_saveWaehlerverzeichnis_when_givenValidModel() {
      ...
      Mockito.when(waehlerverzeichnisModelMapper.toEntity(modelToSave))
          .thenReturn(mockedMappedEntity);

      Assertions.assertThatNoException()
          .isThrownBy(() -> unitUnderTest.setWaehlerverzeichnis(modelToSave));
    ...
    void should_throwTechnischeWlsException_when_savingFailed() {
      ...
      val mockedWlsException =
          TechnischeWlsException.withCode("000").inService("service").buildWithMessage("message");

      Mockito.when(waehlerverzeichnisModelMapper.toEntity(modelToSave))
          .thenReturn(mockedMappedEntity);
      Mockito.doThrow(mockedSaveException)
          .when(waehlerverzeichnisRepository)
